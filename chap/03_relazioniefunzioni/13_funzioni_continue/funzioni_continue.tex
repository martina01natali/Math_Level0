...
\begin{teorema}[Teorema del punto critico]
Una funzione continua 
definita in un intervallo chiuso 
che ha un massimo in un suo punto interno 
derivabile, 
in quel punto ha derivata nulla.
\end{teorema}
...
\[\forall x \in \IR \quadra{x} \geqslant x \geqslant \quadra{x}+1\]
\end{enumerate}
...
\[\left[a;~a+t \right[,~[a+t;~a+2t[,~\dots,~
[a+jt;~a+(j+1)t[,~\dots,~
[a+\tonda{n-1}t;~a+nt=b]\]

Gli estremi di questi intervalli sono chiamati punti di partizione 
dell'intervallo:
\[a;~a+t;~a+2t;~a+3t;~\dots;~a+\tonda{n-1}t;~a+nt=b\]
...
\[f(a+k\delta)<0 \leqslant f(a+(k+1)\delta)\]
...
funzione non dice nulla sul numero degli zeri.
...
\begin{proof}
Suggerimento: 
supponiamo che \(f(a) < h \leqslant f(b)\), 
...
\begin{enumerate}[nosep]
 ...
 allora \(f(x)<0 \forall x \in \intervcc{a}{b}\);
 \item se \(f(c)>0\) per un qualunque \(c \in \intervcc{a}{b}\)
 allora \(f(x)>0 \forall x \in \intervcc{a}{b}\).
\end{enumerate}
\end{corollario}
...
\begin{proof}
Posti \(x_0 < x_1 \forall x_0, x_1 \in I\) per il teorema della pendenza 
media:
\[f'(c) = \dfrac{f(x_1)-f(x_0)}{x_1-x_0}\]
dato che 
\[\forall c \in I \Rightarrow f'(c) > 0 \wedge x_1 - x_0 > 0\]
si ha:
\[\dfrac{f(x_1)-f(x_0)}{x_1-x_0}>0 \Rightarrow f(x_1)-f(x_0)>0
