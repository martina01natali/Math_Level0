...
\begin{inaccessibleblock}
[Immagine di una porzione dell'insieme di Mandelbrot.]
\includegraphics[scale=0.45]{img/Mandelbrot_Set-10-SATELLITE_VALLEY-large.jpg}
\end{inaccessibleblock}
...
\[x > 0 \text{ tale che } x < \frac{1}{n} \text{ per qualunque } n \in \N\]

tradotto in simboli:

\[x > 0 \quad | \quad x < \frac{1}{n} \quad \forall n \in \N\]
...
\[\text{se} \quad x < \frac{1}{n} \quad \forall n \in \N 
\quad \text{allora} \quad \frac{1}{x} > n \quad \forall n \in \N\]
...
Possiamo vedete questo insieme anche come formato dai seguenti elementi:
...
\subsubsection{Microscopio non standard}
...
\subsubsection{Telescopio non standard}
\label{subsec:insnum_microscopio}
...
\subsubsection{Grandangolo non standard}
\label{subsec:insnum_microscopio}
...
% % }
% 
% \newcommand{\intestaz}[1]{\foreach \e in #1 {p\{1.2cm\}|}}
% 
% \newcommand{\riga}[1]{\foreach \e in #1 {\centra{#1}}}
% 
% \newcommand{\tabellina}[1]{
% \begin{center}
% % \begin{tabular}{\foreach \e in #1 {p{1.2cm}|}}
% \begin{tabular}{#1}
...
% }
% 
% \elenconumerato{{a,s,d,f,g}}
% 
% \intestaz{{a,s,d,f,g}}
% \riga{{$+$}, 0, inn, fni, I}
% 
% \tabellina{p{1.2cm}|p{1.2cm}|p{1.2cm}|p{1.2cm}|p{1.2cm}|}
% 
% % \tabellina{\intestaz{{a,s,d,f,g}}}
...
infinitamente vicini). Perché siano indistinguibili devono differire di un 
infinitesimo di ordine superiore.

Due numeri infiniti possono essere indistinguibili anche se differiscono di un 
valore finito o addirittura infinito. Due infiniti sono indistinguibili se 
