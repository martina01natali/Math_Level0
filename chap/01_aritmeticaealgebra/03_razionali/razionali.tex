...
\chapter{Numeri razionali (e irrazionali)}

\section{Premessa storica}
\label{sec:03_origine}
...
\input{\folder lbr/fig001_horus.pgf}
\end{wrapfigure}

Alcune unità frazionarie più comuni venivano indicate con le parti dell'occhio 
di Horus; secondo la leggenda Horus, nella lotta contro lo zio Seth, 
reo di avergli ucciso il padre, perse un occhio le cui parti vennero ritrovate 
...
raggruppamento
\[\frac{2}{3}=\bigg\lbrace\frac{2}{3},\frac{4}{6},\frac{6}{9},\frac{10}{15},
\frac{14}{21},\ldots\bigg\rbrace.\]
...
Per lo stesso motivo~$1,1\overline{9}=2$,~$21,22\overline{9}=21,23$.
...
si trovano spesso a doversi confrontare con misurazioni di grandezze espresse 
...
Come si può notare, per scrivere il risultato di un'operazione tra due numeri 
in questo caso `molto piccoli', è necessario fare particolare attenzione in 
quanto, per l'eccessiva quantità di cifre decimali, è facile commettere degli 
errori.

Per risolvere questo problema, si preferisce utilizzare una scrittura compatta 
...
\[\alpha=k\cdot10^n,\] 
dove~$k$ è un numero decimale maggiore o uguale a~$1$ e minore di~$10$ e~$n$ 
è un numero intero.
\end{definizione}
...
Per esprimere questa misura in notazione scientifica basta considerare la sua 
frazione generatrice, ovvero:
\[0,000007\unit{m}=7\cdot\frac{1}{1000000}\unit{m}=7\cdot10^{-6}\unit{m}.\]

Allo stesso modo il numero~0,000000026 viene scritto in notazione scientifica 
come segue:
\[0,000000026=2,6\cdot\frac{1}{100000000}=2,6\cdot\frac{1}{10^8}=2,6\cdot10^{-8}.\]
...
Consideriamo ora la misura del raggio della Terra, ovvero~$6\,400\,000\unit{m}$,
...
\begin{enumeratea}
 ...
 \item scrivere la moltiplicazione tra il numero ottenuto al passo precedente
  e dieci elevato ad un esponente pari al numero si spostamenti della virgola 
...
\begin{exrig}
 ...
 posizione della virgola:~348\,000\,000\,000\,000,0.
  \paragraph{Passo~$a$} Per ottenere un numero con una sola cifra diversa 
   da zero a sinistra della virgola devo spostare la virgola di~14 posti 
   verso sinistra;
  \paragraph{Passo~$b$} ora scrivo la moltiplicazione tra il numero 
   ottenuto:~$3,48$~e~10 elevato alla~14:~$3,48 \cdot 10^{14}$.
 \end{esempio}
 \begin{esempio}
  Scrivi 0,0000340 in notazione scientifica.
  \paragraph{Passo~$a$} Per ottenere un numero con una sola cifra diversa 
   da zero a sinistra della virgola devo spostare la virgola di~5 posti 
   verso destra;
  \paragraph{Passo~$b$} ora scrivo la moltiplicazione tra il numero 
   ottenuto:~$3,40$~e~10 elevato alla~$-5$:~$3,40 \cdot 10^{-5}$.
 \end{esempio}
 \begin{esempio}
...
\officialeuro\ 21~$\displaystyle:\frac{3}{5}=$ \officialeuro\ 21$\cdot\dfrac{5}{3}=$ \officialeuro\ 35.
 ...
 \begin{esempio}
Giuseppe possiede \officialeuro\ 150. Se spende i~$3/5$ della somma posseduta e 
...
% \ovalbox{\risolvii \ref{ese:3.75}, \ref{ese:3.76}, \ref{ese:3.77}, \ref{ese:3.78}}
...
% \vspazio\ovalbox{\risolvii \ref{ese:3.115}, \ref{ese:3.116}, 
% \ref{ese:3.117}, \ref{ese:3.118}, \ref{ese:3.119}, \ref{ese:3.120},
% \ref{ese:3.121}, \ref{ese:3.122}, \ref{ese:3.123}, \ref{ese:3.124}, 
% \ref{ese:3.125}}
% 
% \vspazio\ovalbox{\ref{ese:3.126}, \ref{ese:3.127}}
...
Quando si deve addizionarre un numero intero ad una frazione si può evitare 
...
frazione con un unico passaggio moltiplicando il numero intero per il denominatore 
e aggiungendolo al numeroatore della frazione.
