...
\newcommand{\microscopio}[6]{% Microscopio messo sopra.
  % Esempio di chiamata:
  % \microscopioa{(8, 5)}{2}{120}{-50}{(7.6, 10)}{\infty}
  \def \punto{#1}
  \def \lung{#2}
  \def \anga{#3}
  \def \angb{#4}
  \def \postimes{#5}
  \def \labeltimes{#6}
  \draw \punto -- +(\anga: \lung)
  arc [start angle=\angb, delta angle=350, radius=2] -- cycle;
  \node at \postimes {\(\times \labeltimes\)};
}
