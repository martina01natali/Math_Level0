...
Nelle espressioni che seguono facciamo uso delle solite conoscenze, che ricordiamo:
\begin{itemize}[nosep]
 \item $M, \ N$ sono iperreali infiniti;
 \item $\epsilon, \ \delta$ sono iperreali infinitesimi;
 \item $a, \ k \in \IR$ sono numeri standard, cioè corrispondono a numeri reali;
 \item la funzione parte standard $\pst{x}$, applicata a un iperreale finito $x$, permette di conoscere quale reale $r$ corrisponda a $x$; 
 \item un iperreale finito $x$ è formato da un reale $r=\pst{x}$ e da un infinitesimo $\delta$: \(x~=~\pst{x}~+~\delta~=~r~+~\delta\). \\
\end{itemize}

Sono prive di dubbi espressioni come \(a^\delta\), \(a^M\), \(log_a\delta\), \(log_a M\), dove \(a>0\) è un numero standard. Per capirlo, basta seguire l'andamento dei grafici contenuti in questo capitolo. 
...
Elenchiamo qui alcuni esempi notevoli, che ritroverai nel capitolo sui limiti, l'anno prossimo.\\
