...
% potenze con esponente intero, ma non possiamo calcolare i valori di 
% \(f:~x \mapsto a^x \stext{con} a<0\) per qualunque \(x \in \R\). 
...
\begin{center}
\fbox{
D'ora in avanti considereremo esponenziali solamente con base \(a > 0\).}
\end{center}
\paragraph{\fbox{\(a>1\)}}
...
% funzione diventa una costante:~\(y=1\). 
...
Immaginiamo che un istituto bancario, assurdamente conveniente, conceda 
...
Sono equazioni di forma: \quad \fbox{\(a^x=b\)} \quad (con \(a>0\))\\[4pt]
...
% Usando il logaritmo si ottiene: 
% \(x=\log_5 10 = 1,4306765580733933\).\\[2pt]
...
\begin{esempio}\(7^{x^2} \div 7^{\,5} - \dfrac{49}{7^{6x}} = 0\)\\[4pt] 
...
\[x^2-5 = 2-6x \quad\Rightarrow\quad \dots\quad \Rightarrow \quad 
...
Quindi la soluzione è:\\[7pt]
 ...
 \insiemesoluzione{\disegno{\raylconasse{0}{7}{3.5}{+6}{blue}{x}}}
                  {\(x \leqslant 6\)}
                  {\(\intervac{-\infty}{+6}\)}
