...
Leggiamo la formulazione della definizione. La somma delle distanze di un generico punto dell'ellisse da due 
punti definiti fuochi risulta sempre pari a \(2a\), qualsiasi sia il punto dell'ellisse (vedi i punti
...
\begin{equation}
\label{eccEllisse1}
e=\dfrac{\text{distanza focale}}{\text{asse  
maggiore}}=\dfrac{2c}{2a}=\dfrac{c}{a}=\dfrac{\sqrt{a^{2}-b^{2}}}{a}
\end{equation}
Dato che \(c<a\), vale la seguente disequazione: \( \displaystyle 0 \leq e \leq 1\)\\[4pt]
Vediamo cosa succede al variare dell'eccentricità tra 0 e 1.
\begin{itemize}
 \item \(e=0\) (caso limite): la distanza focale è nulla, i due fuochi coincidono nel centro e 
...
crescere di \(e\), determinando quindi l'allungamento dell'ellisse sull'asse \(x\)
...
risultano quindi:
\( A_{1} (5;~0)\) e \( A_{2} (-5;~0)\),  \( B_{1} (0;~4)\) e \( B_{2} (0;~-4)\). \\[7pt]
\begin{minipage}{.6\textwidth}
Determiniamo ora semidistanza focale e fuochi:\\[6pt] c=\( \sqrt{25-16} =3 \quad\Rightarrow \quad F_{1} (3;~0)\) e 
\( F_{2} (-3;~0)\). \\[5pt] Eccentricità: \quad \( e= \dfrac{3}{5} =~0,6\).\\[6pt]
...
\begin{minipage}{.4\textwidth}
  %    \begin{inaccessibleblock}[Cono a due falde tagliato da un piano
  %      che forma un'ellisse.]
  \includegraphics[width=0.8\textwidth]{img/esempio1.jpg}
  %    \caption{Generazione di un cono a due falde}% 
...
\noindent Le coordinate dei fuochi possono essere riscritte in funzione di \(a\) e \(b\): \(F_{1,2} \punto{0}{\pm\sqrt{b^{2}-a^{2}}}\)
\\[4pt]
...
\(a\) e \(b\), e dunque occorrono due condizioni per poter scrivere l'equazione di una 
determinata ellisse.
