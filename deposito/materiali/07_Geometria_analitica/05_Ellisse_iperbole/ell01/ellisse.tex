...
Leggiamo la formulazione della definizione. La somma delle distanze di un 
generico punto dell'ellisse da due 
punti definiti fuochi risulta sempre pari a \(2a\), qualsiasi sia il punto 
dell'ellisse (vedi i punti
\(P_{1}\), \(P_{2}\) e \(P_{3}\) della Figura \ref{fig:coniche}). Questa 
lunghezza (\(2a\)) è associata ad un numero reale positivo che non può 
...
\label{subsec:ellisse_caratteristiche}

Studiamo innanzitutto le caratteristiche dell'ellisse con i fuochi appartenenti 
all'asse \(x\).

\begin{description}
\item [\textbf{Intersezioni con gli assi}]: per determinarle, è sufficiente 
porre a sistema l'equazione dell'ellisse con le 
...
chiaro ora il significato dei parametri \(a\) e \(b\): \(a\) è il 
\textbf{semiasse maggiore} 
dell'ellisse relativo alla sua dimensione maggiore, ,mentre \(b\) è il 
\textbf{semiasse 
...
due assi è posta nell'origine, evidenzia una doppia simmetria (rispetto 
all'asse \(x\) e \(y\)).
In generale è possibile dimostrare che un'ellisse è simmetrica rispetto alla 
retta che congiunge i 
...
\item [\textbf{Relazioni tra i parametri}]: ci chiediamo ora qual è la 
relazione 
tra i parametri visti cioè il semiasse maggiore \(a\), il semiasse minore \(b\) 
e 
l'ascissa del fuoco \( F_{1} \), ovvero \(c\). Per ricavare questo non serve 
altro che 
...
proprietà: la distanza tra il vertice \(B_1\) e il fuoco è uguale ad \(a\) 
...
\item [\textbf{Eccentricità}] il rapporto tra la distanza focale e la lunghezza 
...
\]
Dato che \(c<a\), vale la seguente disequazione: \( \displaystyle 0 \leq e \leq 
1\)\\[4pt]
Vediamo cosa succede al variare dell'eccentricità tra 0 e 1.
\begin{itemize}
 \item \(e=0\) (caso limite): la distanza focale è nulla, i due fuochi 
coincidono nel centro e 
quindi l'ellisse rappresenta una circonferenza di raggio \(a\)
\item \(0<e <1\): i due fuochi sono distinti e in particolare si allontanano al 
crescere di \(e\), determinando quindi l'allungamento dell'ellisse sull'asse 
\(x\)
\item \(e=1\) (caso limite):
l'asse maggiore coincide con la distanza focale \(a=c\) e l'ellisse 
...
\begin{minipage}{.39\textwidth}
     \begin{inaccessibleblock}[Ellisse con fuochi sull'asse x.]
...
\end{minipage}
 \hfill
 \begin{minipage}{.39\textwidth}
      \begin{inaccessibleblock}[Ellisse con i fuochi sull'asse y.]
...
%\begin{esempio}
%Determinare l'equazione dell'ellisse avente \(a=11\) e \(b=8\).\\[1pt]
%
%Calcoliamo \( a^{2} \) e \( 
%b^{2} \) ed inseriamo nell'equazione canonica:\quad \( 
