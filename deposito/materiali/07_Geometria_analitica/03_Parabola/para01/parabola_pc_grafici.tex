...
\newcommand{\parabolaerette}{% 
    ...
    \node at (-5.5, -5.3) {t};
    \node at (-4.2, -6.5) {s};
    \filldraw (-1, 3) circle(2pt) (-3, -5) circle(2pt) (1, 3) circle(2pt);
    \end{scope}
    }
}
...
\newcommand{\intersezioniparabole}{% 
    ...
    \tkzFct[domain=-3.3:+5.3, ultra thick, color=Red!50!black]
           {5./4.*x*x-19./4.*x-1}
    \begin{scope}[violet!70!black]
    \filldraw (-1, +5) circle (2pt) node [xshift=-9pt] {\(I_0\)} 
              (+4, 0) circle (2pt) node [xshift=9pt, yshift=6pt] {\(I_1\)};
    \end{scope}
    }
}
