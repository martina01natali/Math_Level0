...
\input{\folder parabola_pc_grafici}

\chapter{La parabola nel piano cartesiano}

% \section{Parabola come luogo geometrico}
% \label{sec:parabola_rappresentazionetrinomio}
% 
% La parabola è il luogo dei punti equidistanti da un punto detto 
% \emph{fuoco} 
% e una retta detta \emph{direttrice}:
% 
% \parabolafuocoedirettrice
% 
% Parabola con coefficiente del termine di secondo grado positivo:
% 
% \parabolina{.5*x*x-1*x-2}{parabola con \(a>0\)}
% 
% Parabola con coefficiente del termine di secondo grado negativo:
% 
% \parabolina{-.5*x*x-1*x+2}{parabola con \(a<0\)}
% 
% Parabola con \(b=0\)
% 
% \parabolina{-.5*x*x+2}{parabola con \(y=ax^2+c \quad \tonda{b=0}\)}
% 
% Parabola con \(c=0\)
% 
% \parabolina{-.5*x*x-1*x}{\(y=ax^2+bx \quad \tonda{c=0}\)}
% 
% Parabola con \(b=0 \text{ e } c=0\)
% 
% \parabolina{-.5*x*x}{\(y=ax^2 \quad \tonda{b=0 \text{ e } c=0}\)}
% \newpage
...
Consideriamo la funzione: \(P(x)=x^2 -2x -2\) chiamiamo \(y\) il valore che 
assume la funzione quando diamo a \(x\) diversi valori. 
Per iniziare a \(x\) diamo dei valori interi ottenendo così 
...
piano cartesiano si dispongono lungo una curva.]
\begin{center} \scalebox{.9}{\puntia} \end{center}
...
\begin{itemize}
\item In <D1> la formula e stata inserita con:
<menu-Inserisci-Oggetto-Formula...> \\
f(x) = ax\^{}2+bx+c;
\item In <B8> La formula è:\\
=B\$3*A8\^{}2+B\$4*A8+B\$5;
% \begin{verbatim}
% =B$3*A8^2+B$4*A8+B$5;
% \end{verbatim}
\item Il grafico è di tipo:
XY (Dispersione), Tipo linea: Liscia, Solo linee;
\item Per ottenerlo così ho lavorato sulle proprietà degli assi e sulla 
larghezza del grafico.
\end{itemize}
}{
\begin{center}
\includegraphics[scale=0.4]{\folder lbr/coefficienti_fogliocalcolo}
\end{center}
}
...
Teniamo presente questa successione e proviamo a disegnare la parabola 
\(y=x^2\).
L'ascissa del vertice della parabola è \(-\dfrac{b}{2}=0\) e l'ordinata è 
\(y=x_V^2=0^2=0\) disegniamo il vertice nel punto \((0;~0)\). 
Ogni volta che \(x\) 
...
\centering
\scalebox{.7}{\disrapido}
  \caption{Parabola: metodo rapido di disegno.} 
  \label{fig:parabola_parabola0}
\end{inaccessibleblock}
\end{minipage}
\end{figure}
\noindent Cosa succede se il coefficiente del termine di secondo grado è 
diverso da~1? 
...
% Applichiamo questa tecnica ai casi precedenti.
...
\affiancati{.49}{.49}{
\begin{enumerate}
 \item il punto è esterno alla parabola: \\
 2 tangenti reali distinte;
 \item il punto appartiene alla parabola: \\
 due tangenti reali coincidenti
  (una tangente?);
 \item il punto è interno alla parabola: \\
 nessuna tangente reale.
\end{enumerate}
\vspace{10mm}

Vediamo i tre casi con degli esempi \ref{fig:parabola_tangenti}.
}{
\begin{inaccessibleblock}[Parabola di equazione \(y=x^2\).]
\centering
\scalebox{.8}{  \parabolaetangenti}
