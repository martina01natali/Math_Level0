...
le 
relazioni tra eventi si usano i corrispondenti simboli che si sono 
utilizzati 
...
\centering\input{\folder lbr/fig001_com.pgf}
...
\end{minipage}\hfil
\begin{minipage}[t]{.45\textwidth}
\centering\input{\folder lbr/fig002_inter.pgf}
...
\vspace{5px}
\begin{minipage}[t]{.45\textwidth}
\centering\input{\folder lbr/fig003_uni.pgf}
\caption{L'\emph{unione} tra gli eventi \(A\) e \(B\) indicata con \(C=A\cup B\) 
è 
l'evento che si verifica quando si verifica almeno uno dei due eventi.}
\end{minipage}\hfil
\begin{minipage}[t]{.45\textwidth}
\centering\input{\folder lbr/fig004_imp.pgf}
\caption{L'evento \(A\) \emph{implica} l'evento \(B\), in simboli \( A \subseteq 
B\), 
se ogni volta che si verifica \(A\) si verifica anche \(B\).}
\end{minipage}
\vspace{5px}
\begin{minipage}[t]{.45\textwidth}
\centering\input{\folder lbr/fig005_inc.pgf}
\caption{Due eventi \(A\) e \(B\) si dicono \emph{incompatibili}, se il 
verificarsi 
dell'uno esclude il verificarsi dell'altro.}
\end{minipage}\hfil
\begin{minipage}[t]{.45\textwidth}
\centering\input{\folder lbr/fig006_esa.pgf}
...
\centering\input{\folder lbr/fig007_par.pgf}
...
\(A\,\cup\,B\,\cup\dots \cup \,F=\Omega \)) e a due a due incompatibili 
(ovvero
\(A\,\cap\,B=A\,\cap\,C=B\,\cap\,C=\dots=\varnothing\)) diremo che essi 
formano una \emph{partizione dello spazio degli 
eventi}.
\end{definizione}
...
testa''. La probabilità di questo evento è quindi \(p=\frac 1 
8=0,125=12,5\%\).
...
dell'unione 
di due eventi. Nel caso precedente, lo spazio degli eventi era lo stesso 
per 
...
\(\Omega 
_2=\{1,2,3,4,5,6\}\). L'evento \((A\cap B)\) ha il seguente insieme degli 
eventi 
elementari: \[ \Omega 
...
cartesiano dello spazio elementare di A moltiplicato per quello di B. Si 
può 
...
=\{(T,1);(T,2);(T,3);(T,4);(T,5);(T,6);(C,1);(C,2);(C,3);(C,4);(C,5);(C,6)\}
 \] 
la probabilità dell'evento intersezione è: \(P(A\cap B)=\frac 2{12}=\frac 1 
6\).
