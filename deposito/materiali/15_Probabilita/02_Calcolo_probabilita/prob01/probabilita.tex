...
\input{\folder probabilita_grafici}
...
Nel lancio di un dado sono possibili sei risultati, espressi dai numeri 
...
Gli eventi elementari di un insieme \(A\) e gli eventi composti che si 
...
% \begin{definizione}
% Diciamo che due eventi sono \emph{incompatibili} se la loro intersezione è 
% l'insieme vuoto:
% \[A\,\cap\,B=\varnothing\]
% \end{definizione}
...
\begin{inaccessibleblock}[Intersezione vuota]
\begin{center} 
% \input{\folder lbr/fig005_inc.pgf} \\
\eventiincompatibili \\
\emph{Eventi incompatibili}
\end{center}
% \caption{Due eventi \(A\) e \(B\) si dicono \emph{incompatibili}, se il 
...
\begin{center} 
% \input{\folder lbr/fig004_imp.pgf} \\
\implicazioneeventi \\
\emph{Implicazione}
\end{center}
% \caption{L'evento \(A\) \emph{implica} l'evento \(B\), in simboli \( A 
% \subseteq B\), se ogni volta che si verifica \(A\) si verifica anche \(B\).}
\end{inaccessibleblock}
\end{minipage}
% \end{figure}
...
\begin{center} 
% \input{\folder lbr/fig006_esa.pgf} \\
\eventiesaustivi \\
\emph{Eventi esaustivi}
\end{center}
% \caption{Due o più eventi si dicono \emph{esaustivi}, se almeno uno di essi 
% si verifica. L'unione di tali eventi coincide con l'insieme \(\Omega\).}
\end{inaccessibleblock}
\end{minipage}
\hfill
\begin{minipage}{.48\textwidth}
\begin{inaccessibleblock}[Figura: TODO]
\begin{center} 
% \input{\folder lbr/fig007_par.pgf} \\
\partizioneeventi \\
...
La probabilità di un evento E è data dal rapporto tra il numero \(f\) dei 
casi 
...
una 
determinata prova e che tale prova sia ripetibile infinite volte nelle 
stesse 
condizioni.
...
% \begin{exrig}
\begin{esempio}
In un'azienda alimentare si producono vasetti di marmellata. In uno studio 
di 
...
in modo che, se fa un incidente, la compagnia paghi per intero il danno?

Per rispondere a queste domande le compagnie di assicurazioni sono in grado 
di stimare, sulla base dei numerosissimi incidenti stradali che si verificano 
ogni anno, qual è la probabilità che un guidatore provochi un incidente 
d'auto.
\end{esempio}
...
% scommettere sul verificarsi di un evento \(E\) a cui attribuiamo 
% probabilità \(p\). 
...
% il 
% guadagno \(\frac q g\) o anche \(q:g\) che si legge \(q\) a \(g\). 
% In questo caso \(q\) e \(g\) si chiamano le \emph{poste} o le \emph{messe} 
% del gioco.
% Che relazione c'è tra questo rapporto e la probabilità?
% 
% Se in un grande numero di scommesse così congegnate vincessimo la somma 
% \(g\) una frazione \(p\) di volte e perdessimo la somma \(q\) una frazione 
% \(1-p\), affinché il 
% gioco risulti equo dovremmo avere \(p\cdot g-q\cdot (1-p)=0\). 
% Isoliamo \(p\) nell'uguaglianza:
% \begin{equation*}
% p\cdot g-q\cdot (1-p)=0 \Rightarrow p\cdot g-q+q\cdot p=0\Rightarrow p\cdot 
% (g+q)=q \Rightarrow p=\frac q{g+q}.
% \end{equation*}
% La relazione è dunque questa: la probabilità di una scommessa \(q:g\) è 
% data dalla perdita q al numeratore e al denominatore la somma complessiva 
% che si incassa data dal guadagno più quello che si è scommesso.
% 
% 
% % \begin{exrig}
% \begin{esempio}
% Supponiamo che la vincita ai mondiali di calcio dell'Italia sia data 
% \(5:12\) 
% cioè 
...
\begin{inaccessibleblock}[Unione di due insiemi]
\begin{center} 
% \input{\folder lbr/fig003_uni.pgf} \\
\unioneeventi
\emph{Unione di due insiemi}
\end{center}
% \caption{L'\emph{unione} tra gli eventi \(A\) e \(B\) indicata con 
% \(C=A\cup B\) è l'evento che si verifica quando si verifica almeno uno dei 
% due eventi.}
\end{inaccessibleblock}
\end{minipage}
% \end{figure}
...
\begin{center}
% \input{\folder lbr/fig008_uni.pgf}\\
\unioneincompatibili
...
% Può essere utile per avere un'idea intuitiva di questa uguaglianza pensare 
% alla 
...
\begin{minipage}{.39\textwidth}
\begin{center}
%  \input{\folder lbr/fig009_int.pgf}\\
 \unionecompatibili
...
\hfill
\begin{minipage}{.39\textwidth}
\begin{inaccessibleblock}[Insieme complementare]
\begin{center} 
% \input{\folder lbr/fig001_com.pgf} 
\eventocomplementare \\
...
% \vspazio\ovalbox{\risolvii \ref{ese:9.39}, \ref{ese:9.40}, 
% \ref{ese:9.41}, 
...
\begin{center} 
% \input{\folder lbr/fig002_inter.pgf} \\
\intersezioneeventi \\
\emph{Intersezione di eventi}
\end{center}
% \caption{L'\emph{intersezione} tra gli eventi \(A\) e \(B\) indicata con 
% \(C=A\cap B\) 
...
Due eventi A e B si dicono \emph{indipendenti} se il verificarsi di A 
...
\begin{inaccessibleblock}[Prodotto di eventi]
\begin{center}
%  \input{\folder lbr/fig010_int.pgf}
\prodottoeventi
\end{center}
\end{inaccessibleblock}
...
\hfil
\begin{minipage}{.59\textwidth}
\begin{inaccessibleblock}[Diagramma ad albero]
 \begin{center}
%  \scalebox{1}{\input{\folder lbr/fig011_alb.pgf}}
 \alberomonetadado
\end{center}
\end{inaccessibleblock}
\end{minipage}
...
\begin{itemize*}
\item \( B_{1}= \) nella prima estrazione pallina bianca 
\(\to P(B_1)=\frac 3 5\)
\item \( B_{2}= \) nella seconda estrazione pallina bianca 
\(\to P(B_2)=\frac 3 5\) 
(estrazione con reimmissione)
in quanto la pallina si rimette nell'urna;
\item \( N_{1}= \) nella prima estrazione pallina nera 
\(\to P(N_1)=\frac 2 5\)
\item \( N_{2}= \) nella seconda estrazione pallina nera 
\(\to P(N_2)=\frac 2 5\)
\end{itemize*}
...
\(P(N_1)=\frac 2 5\) con la probabilità di avere nera nella seconda estrazione 
\(P(N_2)=\frac 2 5\) 
...
indipendenti.
\begin{center}
%  \scalebox{.7}{\input{\folder lbr/fig012_alb.pgf}}
\scalebox{.7}{\alberopalline}
\end{center}
Le domande che posso fare su questo esperimento sono relative allo spazio 
degli 
eventi \(\wp (\Omega ).\) ove 
\(\Omega=\{(B_{1,}B_2);(B_{1,}N_2);(N_{1,}B_2);(N_{1,}N_2)\}\) sono del tipo 
``Quale è la probabilità che escano palline di diverso colore'', 
``Qual è la probabilità che la prima pallina sia bianca'', ecc.
\end{esempio}
% \end{exrig}
...
% \vspazio\ovalbox{\risolvii \ref{ese:9.47}, \ref{ese:9.48}, 
% \ref{ese:9.49}, 
...
% \newpage
\begin{center}
 \input{\folder lbr/fig013_alb.pgf}
\end{center}
...
\begin{center}
 \input{\folder lbr/fig014_cond.pgf}
\end{center}
...
conviene scommettere altrimenti no.
...
estrazioni?
\end{comment}
% \vspazio\ovalbox{\risolvii \ref{ese:9.60}, \ref{ese:9.61}, 
% \ref{ese:9.62}, 
