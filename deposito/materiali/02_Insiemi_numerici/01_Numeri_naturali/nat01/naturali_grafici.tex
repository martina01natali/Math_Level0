...
\begin{tikzpicture} 
  \node (esp) at (0,0) 
    {$2^3=1 \times \underbrace{2\times 2\times 2}_{3 \text{ volte}} = 8$};
\begin{scope}[font=\scriptsize]
  \draw[<-]  (-1.4,.5)--(-1.2,.7) node[right] {esponente};
  \draw[->]  (-2.0,-.1)--(-1.8,.1) node[below left=.2] {base};
  \draw[<-]  (1.8,.1)--(2.0,-.1) node[below right] {potenza };
\end{scope}
\end{tikzpicture}
}
...
%   \node (a) [above=of divisione-3-6.north east] {1.};
% Seconda divisione
