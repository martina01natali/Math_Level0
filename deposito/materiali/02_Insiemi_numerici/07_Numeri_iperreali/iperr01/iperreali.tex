...
dimostrate (nella parte che segue \(x,\ y\) sono iperreali non infiniti):
\begin{multicols}{2}
\begin{enumerate} [nosep]
 \item \(\pst{-x} = -\pst{x}\)
...
Dimostriamole, chiamando \(a=\pst{x} ~ e ~ b=\pst{y}\).
\begin{enumerate}[nosep]
 \item \(\pst{-x}=\pst{-a +\epsilon}=-a=-\pst{x}\)
 \item \(\pst{x \pm y} = \pst{a+\epsilon \pm b+\delta}= \pst{a\pm b+\epsilon \pm \delta}=
 a\pm b = \pst{a}\pm\pst{b}\)
 \item\(\pst{xy} = \pst{(a+\epsilon)(b+\delta)}=\pst{ab+a\delta+b\epsilon+\epsilon\delta}=ab=\) \dots
 \item Se vale la tesi, \(\frac{x}{y}\approx \frac{a}{b}\) perciò la loro differenza sarà un infinitesimo:\\
 \(\dfrac{x}{y}- \dfrac{a}{b}=\dfrac{a+\epsilon}{b+\delta}-\dfrac{a}{b}=
 \dfrac{(a+\epsilon)b-a(b+\delta)}{(b+\delta)b}=\dfrac{ab+b\epsilon-ab-a\delta}{b^2+b\delta}=\dfrac{b\epsilon-a\delta}{b^2+b\delta}\), quindi \dots
 \item (continua tu, bastano poche considerazioni anche senza calcoli).
\end{enumerate}
