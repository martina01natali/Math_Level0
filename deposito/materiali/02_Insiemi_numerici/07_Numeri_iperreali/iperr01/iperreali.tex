...
matematici, perché sono abituati da più di un secolo e mezzo a procedimenti più 
impegnativi e sofisticati.
La ragione per la quale noi invece ne faremo uso è che questi nuovi numeri ci 
rendono il calcolo più semplice e immediato, senza per questo nuocere al rigore 
e alla precisione dei ragionamenti.
...
Dimostriamole, chiamando \(a=\pst{x} ~ \text{e} ~ b=\pst{y}\).
\begin{enumerate}%[nosep]
...
\subsection{Pendenza e tangenti}
...
% \end{enumerate}
% Il punto più complicato è il terzo ma proprio qui il calcolo con gli 
% infinitesimi ci può aiutare. \\
% La pendenza di una retta è:
% \(m=\frac{\Delta y}{\Delta x}=\frac{y_1-y_0}{x_1-x_0}\)
% la stessa cosa vale per una curva, dove però
% \(x_1 \approx x_0 ~ \text{e} ~ y_1 \approx y_0\).
% Quindi la pendenza di una curva in un suo punto \(x_0\) è data da:
% \end{minipage}
% \hfill
% \begin{minipage}{.49\textwidth}
% \begin{center}\pendenza\end{center}
% \end{minipage}
% \[m=\dfrac{df(x_0)}{dx}=
%     \dfrac{f\tonda{x_0+\epsilon}-f\tonda{x_0}}{\tonda{x_0+\epsilon}-x_0}=
%     \dfrac{f\tonda{x_0+\epsilon}-f\tonda{x_0}}{\epsilon}\]
% 
% 
% \begin{center}\ipertangente\end{center}
