...
%  \input{\folder lbr/fig014_g0.pgf}
 \disegno{\rayrconasse{0}{10}{5}{0}{white}{x}}
\end{center}
...
%  \input{\folder lbr/fig015_l0.pgf}
 \disegno{\raylconasse{0}{10}{5}{0}{white}{x}}
\end{center}
...
, quando invece il polinomio assume valori 
negativi, le rette si trovano sotto all'asse~\(x\)% (tratto rosso)
.

Così le due funzioni:~\(f(x) = -4 x +4\) \quad e \quad
\(g(x) = \dfrac{3}{2} x +3\) 
hanno le seguenti rappresentazioni nel piano cartesiano:
...
frazione, il suo denominatore deve essere \emph{diverso da zero}. 
Quindi non sono mai accettabili i valori di \(x\) che rendono nullo il 
denominatore,
per ricordarci di questo, nel grafo dei segni, li indichiamo con una 
\emph{crocetta} invece che con un cerchietto.
...
\begin{itemize} [noitemsep]
 \item Studio del segno del primo fattore~\(num\):\\
 \begin{minipage}{.45\textwidth}
 ...
 \begin{minipage}{.3\textwidth}
  \segnidec{-\dfrac{4}{3}}
 \end{minipage}
 \item Studio del segno del secondo fattore~\(den\):\\
 \begin{minipage}{.45\textwidth}
...
\begin{itemize} [noitemsep]
 ...
 \end{minipage}
 \begin{minipage}{.3\textwidth}
%   \input{\folder lbr/fig060_retta20.pgf}
  \segnicre{+2}
 \end{minipage}
 \item segno di \((-2x+1)\)\\
 \begin{minipage}{.45\textwidth}
...
%   \input{\folder lbr/fig063_grafosegni04.pgf}
...
\[\sistema{x > 0\\ 2x -1 \le \frac{1}{2}x +2} \quad \sRarrow
...
nel primo riportiamo i segni ed eseguiamo il prodotto di segni, 
nel secondo riportiamo gli intervalli e eseguiamo l'intersezione tra 
insiemi.
\begin{center}
\begin{tabular}{ll}
...
\begin{itemize} [noitemsep]
 ...
 \item se~\(2 k - 1\) è uguale a zero, la funzione associata è costante;
