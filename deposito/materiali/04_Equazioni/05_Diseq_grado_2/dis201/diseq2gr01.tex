...
\begin{inaccessibleblock}[Parabola con evidenziate le sue intersezioni con 
gli assi]
\begin{center}
  \parabola
\end{center}
...
% supporre ch eil coefficiente di $x^2$, cioè il coefficiente a, sia 
positivo. 
% Se 
% così non fosse basterebbe moltiplicare per (-1)entrambi i membri della 
% disequazione e cambiare segno a tutti gli addendi. Nell'esempio appena 
svolto 
% da 
...
% essendo 
% $y=3x^2-2x$ l'equazione della parabola associata.La parabola associata ha 
le 
% stesse intersezioni con l'asse delle x di quella dell'esempio, ma non la 
% concavità che in questo caso è rivolta verso l'alto , infatti 
$a=3>0$.Riporta 
...
% discriminante dell'equazione associata $kx^2+3x-7=0$. Il primo coefficiente 
è 
...
\begin{inaccessibleblock}[Retta con gli intervalli evidenziati]
   \input{\folder lbr/fig064_diseq07.pgf}
\end{inaccessibleblock}
  \end{minipage}
...
eseguiamo 
i calcoli per portarla alla forma $p(x)\ge 0$. Si ottiene $3x^3-8x^2-3x\ge 0$ 
e 
...
\begin{esempio}
Un numero è tale che sottraendo al suo cubo il suo triplo si ottiene un 
numero 
maggiore del triplo del suo quadrato aumentato di $ 4 $. Determinare 
l'insieme 
soluzione del problema.

La richiesta del problema implica la ricerca dell'Insieme Soluzione della 
disequazione $x^3-3x>3x^2+4$, di terzo grado nella variabile $x$. Scriviamo 
la 
...
Determiniamo il segno dei singoli fattori: primo fattore $f_1>0\to x>4$ 
secondo 
...
parabola volge la concavità verso l'alto e non ha zeri reali dunque il 
secondo 
fattore è positivo per qualunque valore reale di $x$. Costruiamo la tabella 
dei 
segni:
...
Il binomio al primo membro è una differenza di quadrati, quindi scomponendo 
si 
ottiene: $64x^6-1=(8x^3-1)(8x^3+1)=(2x-1)(4x^2+2x+1)(2x+1)(4x^2-2x+1)$.

Si tratta allora di studiare il segno dei singoli fattori: $f_1>0\to x>\frac 
1 
2$ $f_2>0\to \forall x\in \insR$ $f_3>0\to x>-\frac 1 2$ $f_4>0\to \forall 
x\in 
...
Il trinomio al primo membro è di quarto grado; sappiamo che con la 
sostituzione 
...
% \item impostiamo la disequazione nella forma $\frac{N(x)}{D(x)}\ge 0$ 
oppure 
...
\begin{itemize}
 ...
 \end{minipage}
 \item Con la regola dei segni calcolo il segno della frazione 
  \input{\folder lbr/fig068_grafosegni03.pgf} 
 \item Quindi i valori di~$x$ che rendono vera la disequazione, cioè i valori
 ...
 \subitem 
  ...
  \end{minipage}
  \begin{minipage}{.30\textwidth}
   \input{\folder lbr/fig069_diseq_fratta.pgf}
  \end{minipage}
 \subitem rappresentazione con i 
...
=1+24=25$, positivo per cui si hanno due soluzioni reali distinte; la 
parabola 
$y=-2x^2+x+3$ ha concavità verso il basso per cui essendo $x_1=-1$ e 
$x_2=\frac 
...
\item l'espressione $E$ è positiva per $x\in A=\left\{x\in \insR | 
-1<x<-\frac 
1 
...
\begin{esempio}
Determiniamo l'Insieme Soluzione della disequazione fratta: $3-\frac 
1{2x+1}\ge 
\frac 1{1-x}$.
\begin{enumeratea}
...
\item impostiamo la disequazione: \ $\frac{-6x^2+2x+1}{(2x+1)\cdot (1-x)}\ge 
0$ 
che ci permetterà di rispondere al quesito posto dal problema;
\item studiamo il segno del numeratore e del denominatore:
\begin{itemize*}
\item segno N: $-6x^2+2x+1\ge 0$ disequazione di secondo grado, quindi 
scritta 
...
\item segno D: $-2x^2+x+1>0$ disequazione di secondo grado; il denominatore 
ha 
...
siano reali e che il prodotto delle stesse sia positivo. Pertanto il problema 
è 
...
Risolviamo separatamente le due disequazioni del sistema; indicati con 
$\IS_1$ 
e 
...
gli 
insiemi soluzioni delle disequazioni risolte e visualizziamo l'insieme 
formato 
dai valori che soddisfano contemporaneamente sia l'una che l'altra: sull'asse 
reale depositiamo i valori numerici trovati e rappresentiamo su righe 
distinte 
i 
...
$x=1$ è uno zero del polinomio quindi con la regola di Ruffini otteniamo 
$d_1$: 
$(x-1)\cdot (2x^2-7x+3)\le 0$. L'equazione di secondo grado $2x^2-7x+3=0$ ha 
soluzioni reali $x_1=\frac 1 2\vee x=3$. Si tratta allora di studiare il 
segno 
dei singoli fattori e di determinare il segno richiesto dopo aver costruito 
la 
tabella dei segni:
...
\item $d_2$: $\frac{x^2+x+1}{x^3-x}\ge 0$ è una disequazione fratta, per 
prima 
...
determinando l'insieme $\IS_1\cap \IS_2\cap \IS_3$ individuabile attraverso 
il 
grafico:
