...
% dire:~\(\insN_0=\Dom=\ID\) \(\insZ_0=\Cod=\IM\). Ma succede che due numeri 
opposti 
...
\((a,b) R (x,y)\) se e solo se~\(a+b=x+y\) & ~\(\insN \times \insN\) & 
\([Ri][Si][Tr]\) 
\\
\bottomrule
\end{tabular}
...
simmetrica});
\item se ``il poligono~\(p_1\) ha lo stesso perimetro di~\(p_2\)'' e ``\(p_2\) 
ha lo 
...
\end{enumeratea}
La \emph{partizione} di~\(X\) è l'insieme i cui elementi sono le classi~\(X_1, 
X_2, 
...
chiama \emph{classe d'equivalenza} ogni sottoinsieme di~\(A\) contenente tutti 
e 
soli gli elementi
...
l'insieme~\(\insN_0\) dei naturali diversi da zero e l'insieme~\(\insZ_0\) 
degli 
interi
...
un numero razionale, quindi~\(\IM\subset\insQ^{+}\cup\{0\}\), pertanto la 
funzione 
non è suriettiva.
 \end{esempio}
...
iniettiva.
 ...
 \begin{esempio}
 \label{ex:D.6}
È assegnata la funzione~\(f:x\in N\rightarrow (x-2)\in\insZ\). In questo caso 
la 
...
\((x-2)\in \insZ\) & \(-2\) & \(-1\) & 0 & \(+1\) & \(+2\) & \(+3\) & \(+4\) & 
\ldots \\
\bottomrule
\end{tabular}
\end{center}

Ogni elemento dell'insieme~\(\insN\) trova il corrispondente in~\(\insZ\) 
elementi 
...
una restrizione su \(\insQ\) e fissiamo~\(\Dom = \ID = \insQ_0\). La 
corrispondenza 
...
Lo stesso fatto avviene se consideriamo l'insieme~\(\insZ\) come dominio e i 
punti 
...
Gli insiemi~\(\insN\) e~\(\insZ\) sono infiniti e la loro caratteristica comune 
è 
...
\end{center}
L'insieme~\(\insQ\) rispetto agli insiemi~\(\insN\) e~\(\insZ\) presenta 
un'altra 
...
ma maggiore di~\(\frac{3}{8}\), come si può verificare trasformando la frazione 
in 
...
L'insieme che si ottiene dall'unione dell'insieme~\(\insQ\) con 
l'insieme~\(\insJ\) 
...
Costruisco il segmento rappresentativo del numero irrazionale~\(\sqrt{2}\), che 
è 
...
prendiamo il segmento~\(KD=OA\) e congiungiamo~\(D\) con~\(O\). Per il teorema 
di 
Pitagora sul
triangolo~\(OKD\) si ha
...
.\]
Puntando il compasso in~\(O\) con raggio~\({OD}\) tracciamo l'arco che incontra 
la 
...
% Per rappresentare gli elementi di~\(\insR\times \insR\) cioè le coppie 
ordinate 
...
% 
% Nella figura~\ref{fig:D.9} è tracciata la costruzione descritta sopra:~\(P\) 
è 
...
% sull'asse~\(x\) coincide con~\(O\), quindi la coppia~\((0;5)\) sarà associata 
al 
% punto~\(R\)
...
% 
% Diremo che~\(R\) è il punto di coordinate~\((\alpha;\beta )\), \(\alpha~\) si 
chiama 
...
% monometrico~\(O_{xy}\), conoscendo le coordinate degli estremi~\(A\) e~\(B\) 
del 
% segmento stesso.
% 
% \begin{figure}[t]
...
% \emph{Obiettivo}:~\(\overline{AB}\).
% 
% \emph{Procedura risolutiva}: tracciando da~\(A\) la parallela all'asse~\(x\) 
e 
% da~\(B\) la parallela all'asse~\(y\)
% si determina il vertice~\(C\) del triangolo rettangolo~\(ABC\) di cui~\(AB\) 
è 
% l'ipotenusa.
...
Se si indica con~\(l\) la lunghezza del lato del triangolo, allora il perimetro 
...
\begin{definizione}
  Due grandezze~\(x\) e~\(y\) si dicono \emph{direttamente proporzionali} se il 
...
\begin{definizione}
  Due grandezze~\(x\) e~\(y\) si dicono \emph{inversamente proporzionali} se il 
...
\emph{negativa} allora l'angolo~\(\alpha\) è \emph{ottuso}. Se \(k=1\) l'angolo 
è 
...
\begin{definizione}
Una funzione espressa dalla formula~\(y=m\cdot x+q\) con~\(m\in \insR\) 
e~\(q\in 
...
proporzionalità diretta (il rapporto~\(y/x\) non è costante!). Per determinare 
la 
formula devo stabilire
il valore di~\(m\) (coefficiente angolare) e di~\(q\).
Dalla tabella individuo il valore~\(q=-2\), infatti per~\(x=0\) si ha~\(y=-2\). 
Per 
determinare~\(m\), sommo
