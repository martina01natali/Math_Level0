...
\input{\folder relazioni_grafici.tex}
...
% \begin{exrig}
\begin{esempio}
Consideriamo gli insiemi~\(A=\) \{Parigi, Roma, Atene\} e~\(B=\) \{Italia, 
Francia, 
...
(elemento di~\(A\)) e come secondo elemento uno stato d'Europa (elemento 
di~\(B\)).

Il predicato binario~\(\Kor\): "essere la capitale di", introdotto 
nell'insieme~\(A 
...
diagrammi Eulero-Venn.
 Collegando con una freccia, ciascun elemento di~\(A\) con la sua forma, 
possiamo 
rappresentare
 con un grafico sagittale la corrispondenza~\(\Kor\): ``essere di forma'', tra 
gli 
insiemi
...
L'insieme~\(G_\Kor\) delle coppie ordinate aventi come primo elemento una 
regione 
...
Ionio); (Puglia, Ionio);
(Puglia, Adriatico); (Molise, Adriatico); (Abruzzo, Adriatico); 
(Emilia-Romagna, 
Adriatico); (Marche, Adriatico); (Veneto, Adriatico); (Friuli Venezia Giulia, 
Adriatico)\}.
Se rappresentiamo questa corrispondenza con un grafico sagittale notiamo che 
non 
...
% come dominio~\(\Dom=\) \{persone italiane viventi\} e come 
codominio~\(\Cod=\) \{gli 
% anni dal~1900 al~2012\}.
% Evidentemente~\(\ID=\Dom\): ogni persona ha un determinato anno di nascita, 
ma più 
...
% scriveremo~\(\IM \subseteq \Cod\). Il grafico sagittale di questa 
corrispondenza è 
...
%  \caption{Corrispondenza \emph{molti a uno}: più persone sono nate nello 
stesso 
% anno.}\label{fig:C.4}
% \end{figure}
% \end{inaccessibleblock}
% 
% \begin{esempio}
% Analizziamo la corrispondenza dell'esempio precedente~\(\Kor:R \rightarrow 
M\) 
% ``essere bagnata/o da'' tra l'insieme delle regioni d'Italia e l'insieme dei 
% mari. 
% 
% \(\ID\subset\Dom\) poiché alcune regioni non sono bagnate da alcun mare. 
Moltre 
...
% dominio~\(\Cod=\) \{città d'Europa\} e il codominio~\(S=\) \{stati 
d'Europa\}. È 
% evidente che~\(\ID\subset\Cod\):
% non tutte le città sono capitali, mentre~\(\IM=\Cod\) in quanto ogni stato ha 
la 
% sua capitale; inoltre due città diverse non possono essere capitali dello 
stesso 
% stato.
...
% \end{inaccessibleblock}
% \begin{esempio}
% Consideriamo, tra l'insieme~\(\N_0\) dei numeri naturali diversi da zero e 
% l'insieme~\(\Z_0\) degli interi relativi diversi da zero, la 
% corrispondenza~\(\Kor\): ``essere il valore assoluto di''.
% Per la definizione di valore assoluto di un intero, possiamo senz'altro 
% dire:~\(\N_0=\Dom=\ID\) \(\Z_0=\Cod=\IM\). Ma succede che due numeri 
opposti 
% hanno lo stesso valore assoluto,
% quindi ogni elemento di~\(\mathbb{N}_0\) ha due immagini, per cui il 
grafico 
...
%  \caption{Esempio di corrispondenza di tipo \emph{uno a 
molti}.}\label{fig:C.7}
% \end{figure}
% \end{inaccessibleblock}
% % \end{exrig}
...
% \emph{univoche}; in esse ogni elemento dell'insieme di definizione ha una 
sola 
...
% % \begin{exrig}
% \begin{esempio}
% Consideriamo la corrispondenza~\(\Kor\) che associa ad ogni persona il suo 
codice 
% fiscale: ogni persona ha il proprio codice fiscale, persone diverse hanno 
codice 
% fiscale diverso.
% Dominio e~\(\ID\) coincidono e sono l'insieme~\(P=\) \{persone\}. Codominio 
e~\(\IM\) 
% coincidono e sono l'insieme~\(F=\) \{codici fiscali\}. Il grafico sagittale 
di 
% questa corrispondenza
% è del tipo \emph{uno a uno}. È di questo tipo il grafico sagittale della 
% corrispondenza che associa ad ogni automobile la sua targa, ad ogni moto il 
suo 
% numero di telaio,
% ad ogni maggiorenne, cittadino italiano, il suo certificato elettorale 
\ldots
% 
% \end{esempio}
% % \end{exrig}
% 
% \osservazione In tutti questi casi la corrispondenza è di tipo \emph{uno a 
uno}, 
% il dominio coincide con l'insieme di definizione e l'insieme immagine 
coincide 
% con il codominio.
% 
% \begin{definizione}
% Una corrispondenza di tipo \emph{uno a uno} in cui~\(\Dom=\ID\) 
e~\(\Cod=\IM\) è 
% detta \emph{corrispondenza biunivoca}.
% \end{definizione}
% 
% %\ovalbox{\risolvii \ref{ese:C.4}, \ref{ese:C.5}, \ref{ese:C.6}, 
\ref{ese:C.7}, 
...
Il termine \emph{relazione} entra molto spesso in frasi del linguaggio 
naturale, 
...
Paolo'', ``l'allungamento di una sbarretta di ferro è in relazione con il 
calore 
fornito'',
...
Sono tutte espressioni che ci danno informazioni di un qualche collegamento 
tra 
gli
...
\begin{definizione}
Si dice \emph{relazione} in un insieme~\(A\) un predicato binario che lega 
due 
...
elementi sono gli argomenti del predicato binario, ossia sono in relazione 
tra 
...
% \ovalbox{\risolvii \ref{ese:B.3}, \ref{ese:B.4}, \ref{ese:B.5}, 
\ref{ese:B.6}}
...
Dal momento che una relazione in un insieme~\(Y\) determina un sottoinsieme 
del 
...
\begin{definizione}
Un \emph{grafo} è un insieme di punti detti nodi e di archi che uniscono 
coppie 
di punti.
\end{definizione}

Abbiamo visto che con un predicato si possono formare alcune proposizioni 
aventi 
rispettivamente come soggetto e
come complemento elementi di un insieme: solo le proposizioni vere 
determinano 
...
Nel diagramma di Eulero-Venn (figura~\ref{fig:B.2}) dell'insieme~\(A = 
\lbrace~3, 
5, 6, 9, 30 \rbrace\)
rappresentiamo la relazione~\(\srel\) = ``essere multiplo di'' collegando 
mediante 
una freccia gli argomenti delle proposizione vere.


Come puoi osservare l'elemento~30 è collegato con una freccia all'elemento~6 
in 
quanto la proposizione: ``30 è multiplo di~6'' è vera, ma non all'elemento~9
poiché la proposizione: ``30 è multiplo di~9'' è falsa; inoltre la punta 
della 
freccia è sul numero~6 in quanto complemento del predicato ``essere 
multiplo'';
...
elemento~\(a\) dell'insieme~\(A\) la proposizione: ``\(a\) è multiplo 
di~\(a\)''.
...
elementi~\(a\), \(h\), \(f\) che con~\(a\) sono in relazione:~\(B_1 = \lbrace 
a, 
h, f 
\rbrace\).
Gli elementi di~\(B\) non sono esauriti, quindi ripetiamo i passi scegliendo 
un 
elemento tra quelli rimasti;
\item scegliamo~\(g\) e formiamo il sottoinsieme~\(B_2\) avente come 
elementi~\(g\) 
...
\item scegliamo~\(c\) e formiamo il sottoinsieme~\(B_3\) avente come 
elementi~\(c\), 
\(e\), \(b\) che con esso sono in relazione:~\(B_3 = \lbrace c, e, b 
\rbrace\).
\end{itemize*}

Abbiamo esaurito gli elementi dell'insieme assegnato. Abbiamo così ottenuto 
tre 
...
l'insieme stesso in un numero di sottoinsiemi~\(X_1, X_2, \ldots, X_n\), 
detti 
...
\end{enumeratea}
La \emph{partizione} di~\(X\) è l'insieme i cui elementi sono le 
classi~\(X_1, 
X_2, 
...
chiama \emph{classe d'equivalenza} ogni sottoinsieme di~\(A\) contenente 
tutti 
e 
soli gli elementi
...
Nell'esempio sopra riportato le classi d'equivalenza sono i sottoinsiemi 
di~\(B\) 
...
% \ref{ese:B.35}, \ref{ese:B.36}, \ref{ese:B.37}, \ref{ese:B.38}, 
\ref{ese:B.39}, 
% \ref{ese:B.40},
...
Attraverso alcuni esempi, vogliamo chiarire le differenze tra i diversi tipi; 
a 
...
\begin{definizione}
Si dice relazione d'ordine parziale \emph{in senso stretto} quando la 
relazione 
...
% \ref{ese:B.47}, \ref{ese:B.48}, \ref{ese:B.49}, \ref{ese:B.50}, 
% \ref{ese:B.51},
...
[Funzione rappresentata come scatole: addizione e sottrazione]
\grafifunzbinarie %\\[0.5em]
\end{inaccessibleblock}
\end{center}
...
\begin{inaccessibleblock}
[Funzione rappresentata come scatole: addizione e sottrazione]
\grafidivisione %\\[0.5em]
\end{inaccessibleblock}
\end{center}

Esistono anche funzioni che danno come risultato una ennupla di numeri. 
...
\begin{inaccessibleblock}
[Funzione divmod: il risultato è una coppia; dà un errore se il divisore è 
zero.]
\grafidivmod %\\[0.5em]
\end{inaccessibleblock}
\end{center}
...
\begin{center}
% Rappresentazione della funzione ``divmod''\\[1em]
\begin{inaccessibleblock}
[Rappresentazione della funzione divmod.]
\funzdivmod %\\[0.5em]
\end{inaccessibleblock}
\end{center}
\end{minipage}
...
Si possono quindi evidenziare due sottoinsiemi formati dagli elementi che 
...
\begin{center}
\begin{inaccessibleblock}
[Rappresentazione di una generica funzione.]
...
Per indicare la legge che fa passare dall'insieme~\(A\) all'insieme~\(B\) 
...
\emph{variabile indipendente}; il corrispondente elemento~\(y =f(x)\) si 
chiama 
...
l'insieme~\(\N_0\) dei naturali diversi da zero e l'insieme~\(\Z_0\) 
degli 
interi
relativi diversi da zero. Questa corrispondenza \emph{non è una
funzione} in quanto \emph{non è} una corrispondenza \emph{univoca}: un 
elemento 
di~\(\N_0\) ha due immagini
...
dominio~\(\Q\): di ogni numero razionale si può
...
non è suriettiva.
 ...
 \begin{esempio}
 \label{ex:D.5}
 Analizziamo la corrispondenza che associa \emph{ad ogni intero} il suo 
valore 
assoluto.
...
il dominio coincide con l'insieme~\(\Z\), pertanto è una
funzione:~\(f: \Z\rightarrow\N\) rappresentata in forma
analitica con~\({y=\valass{x}}\) con~\(x\in\Z\) e~\(y=f(x)\in\N\).
...
iniettiva.
 ...
 \begin{esempio}
 \label{ex:D.6}
È assegnata la funzione~\(f:x\in N\rightarrow (x-2)\in\Z\). In questo caso 
la 
...
\begin{tabular}{l*8{c}}
 \toprule
\(x\in \N\) & 0 & 1 & 2 & 3 & 4 & 5 & 6 & \ldots \\
\((x-2)\in \Z\) & \(-2\) & \(-1\) & 0 & \(+1\) & \(+2\) & \(+3\) & \(+4\) 
& 
\ldots \\
\bottomrule
\end{tabular}
...
Ogni elemento dell'insieme~\(\N\) trova il corrispondente in~\(\Z\) 
elementi 
diversi del dominio hanno immagini diverse pertanto la
funzione è \emph{iniettiva}; il codominio o insieme immagine è un 
sottoinsieme 
proprio di~\(\Z\) e precisamente~\(\Cod = IM =\{y\in\Z/y\geqslant 
-2\}\),
pertanto la funzione non è suriettiva.
 ...
 \begin{esempio}
 \label{ex:D.7}
 Analizziamo la corrispondenza:~\(f_{1}:x\in \N\rightarrow (x-2)\in N\) e 
...
\toprule
\(x\in \N\) & 0 & 1 & 2 & 3 & 4 & 5 & 6 & \ldots \\
\((x-2)\in \N\) & & & 0 & 1 & 2 & 3 & 4 & \ldots \\
\bottomrule
\end{tabular}
\end{center}

Fissiamo allora come dominio un sottoinsieme di~\(\N\) e precisamente~\(\Dom 
= 
\ID = \N - \{0,1\}\),
...
una restrizione su \(\Q\) e fissiamo~\(\Dom = \ID = \Q_0\). La 
corrispondenza 
è una funzione tra~\(\Q_0\)
e~\(\Q\). In simboli matematici~\(f: y=\frac{1}{x}\).
 \end{esempio}
% \end{exrig}
...
È assegnata la funzione~\(f:\R\rightarrow \R\) descritta mediante le 
istruzioni
\begin{center}
...
percorrendo le istruzioni con le operazioni inverse otteniamo il valore 
di~\(x\)
...
determinare la controimmagine (cioè l'unico valore di~\(x\) tale che~\(f(x) = 
y\)).
...
funzione~\(f(g(x))\) si ottiene facendo agire prima la~\(g(x)\) che aumenta 
di~1
...
Nello studio degli insiemi numerici abbiamo visto come si possono depositare 
su 
una semiretta i numeri naturali;
la legge costruttiva di questa rappresentazione genera tra 
l'insieme~\(\N=\{0,1,2,3,4,\ldots \}\) e i punti della
semiretta una corrispondenza avente come dominio~\(\N\) e come codominio i 
punti della semiretta.
Ad ogni numero naturale possiamo far corrispondere un punto della semiretta, 
ma 
\emph{non tutti i punti} della semiretta
\emph{sono immagine} di un numero naturale: la \emph{corrispondenza non è 
biunivoca}.

Lo stesso fatto avviene se consideriamo l'insieme~\(\Z\) come dominio e i 
punti 
...
Ad ogni numero intero possiamo far corrispondere un punto della retta 
orientata, 
...
Gli insiemi~\(\N\) e~\(\Z\) sono infiniti e la loro caratteristica comune 
è 
che tra due naturali consecutivi o tra due interi consecutivi
non possiamo trovarne un altro. Si dice che~\(\N\) e~\(\Z\) sono due 
\emph{insiemi discreti}.

Consideriamo ora l'insieme~\(\Q\) dei numeri razionali; sappiamo che anche 
...
\end{center}
L'insieme~\(\Q\) rispetto agli insiemi~\(\N\) e~\(\Z\) presenta 
un'altra 
...
Osserviamo la figura precedente: fra~\(\frac{3}{8}\) e~\(\frac{3}{2}\) si 
trova 
certamente il numero~\(1\).
Costruiamo il numero~\(q=\frac{1}{2} \cdot 
\left(\frac{3}{8}+\frac{3}{2}\right)\) 
ottenuto dividendo per due la somma dei due numeri estremi
dell'intervallo considerato, si ottiene~\(q=\frac{15}{16}\) che è minore 
di~\(1\) e, 
a maggior ragione, minore di~\(\frac{3}{2}\),
ma maggiore di~\(\frac{3}{8}\), come si può verificare trasformando la 
frazione 
in 
...
che risulta maggiore di~\(\frac{3}{8}\) e minore di~\(q\). Con questo 
procedimento, 
che non ha mai termine, possiamo determinare
infiniti altri numeri razionali compresi tra~\(\frac{3}{8}\) 
e~\(\frac{3}{2}\).
\begin{center}
...
cioè che esista una corrispondenza biunivoca tra l'insieme~\(\Q\) e i 
punti 
della retta.
Invece, no! Nel capitolo sull'introduzione ai numeri reali abbiamo visto che 
benché l'insieme~\(\Q\) sia infinito e denso,
...
L'insieme che si ottiene dall'unione dell'insieme~\(\Q\) con 
l'insieme~\(\insJ\) 
degli irrazionali è l'insieme~\(\R\) dei numeri reali, cui
...
\begin{definizione}
Si chiama \emph{ascissa di un punto} sulla retta reale il numero 
reale~\(\alpha\) 
...
% \begin{exrig}
 \begin{esempio}
Determinare l'immagine del numero reale~\(\alpha =1+\sqrt{2}\) sulla retta 
reale.

Fisso la retta orientata e un suo punto~\(O\) al quale attribuisco 
ascissa~\(0\)
...
Costruisco il segmento rappresentativo del numero irrazionale~\(\sqrt{2}\), 
che 
è 
...
.\]
Puntando il compasso in~\(O\) con raggio~\({OD}\) tracciamo l'arco che 
incontra 
la 
...
(figura~\ref{fig:D.4}), permette di ottenere i segmenti di 
misura~\(\sqrt{n}\) 
con~\(n\in \N_{0}\).
Si inizia con la costruzione del triangolo rettangolo isoscele di 
cateto~\(1\) 
...
% le coppie ordinate tali che il primo elemento appartenga ad~\(A\) e il 
secondo 
...
% Preso l'insieme~\(\R\) dei numeri reali, costruiamo il prodotto 
% cartesiano~\(\R\times \R\): esso è costituito dall'insieme delle coppie
% ordinate tali che il primo elemento sia un numero reale come pure il 
secondo 
% elemento. In~\(\R\times \R\) avremo coppie il cui primo
% elemento è~\(0\), coppie il cui primo elemento è un numero positivo e 
infine 
% coppie il cui primo elemento è un numero negativo,
% coppie che possiamo sinteticamente rappresentare nel seguente modo:
% \begin{equation*}
% \R\times \R=\{(0;0),(0;+),(0;-),(+;0),(-;0),(+;+),(+;-),(-;+),(-;-)\}.
% \end{equation*}
% È possibile dare una rappresentazione grafica di questo insieme di infiniti 
% elementi?
% 
% Consideriamo sul piano una coppia di rette perpendicolari, indichiamo 
con~\(O\) 
...
% sinistra a destra, sulla verticale dal basso all'alto) e infine scegliamo 
un 
...
% immagine di zero,
% i punti alla sua destra rappresentano i numeri reali positivi, quelli alla 
sua 
% sinistra tutti i numeri
% reali negativi; analogamente sull'asse delle ordinate il punto~\(O\) è 
immagine 
% dello zero, sopra di questo si collocano i numeri
% positivi e sotto i numeri negativi (figura~\ref{fig:D.8}).
% Per rappresentare gli elementi di~\(\R\times \R\) cioè le coppie 
ordinate 
...
% 
% Nella figura~\ref{fig:D.9} è tracciata la costruzione descritta 
sopra:~\(P\) 
è 
% il punto del piano immagine della coppia~\((2;3)\) e~\(Q\)
% è il punto immagine della coppia~\((-1;1)\). Rappresenta le 
coppie~\((4;-1)\) 
% e~\((-4;1)\).
...
%  \end{figure}
% 
%  Il punto~\(O\) è immagine della coppia~\((0;0)\) ed è chiamato 
\emph{Origine}.
% \paragraph{Prima conclusione:} ogni coppia di numeri reali è rappresentata 
da 
% un punto del piano dotato di riferimento cartesiano ortogonale
% monometrico.
% 
% Prendiamo ora un punto~\(R\) (figura~\ref{fig:D.11}) del piano sul quale 
sia 
% stato fissato un riferimento cartesiano ortogonale monometrico e tracciamo 
% da~\(R\)
% la parallela all'asse~\(y\) che interseca l'asse~\(x\) nel punto~\(A\). A 
questo 
...
% 
% Diremo che~\(R\) è il punto di coordinate~\((\alpha;\beta )\), \(\alpha~\) 
si 
chiama 
...
% di numeri reali.
% 
% In conclusione, esiste una corrispondenza biunivoca tra 
l'insieme~\(\R\times 
% \R\) e l'insieme dei punti del piano dotato di
% riferimento cartesiano ortogonale monometrico. Possiamo dunque 
``confondere'' 
% coppia di numeri reali con punto del piano e
% anzi diremo, secondo gli esempi precedenti, ``\(P\) è il punto~\((2;3)\), 
\(Q\) il 
...
% un procedimento matematico per descrivere enti geometrici attraverso 
numeri, 
% equazioni, disequazioni e tradurre le relazioni
% tra elementi della geometria in relazioni tra enti dell'algebra.
% 
% La geometria analitica tratta quindi questioni geometriche con metodi di 
tipo 
% algebrico.
% 
...
% Assegnato nel riferimento cartesiano ortogonale il punto~\(P(\alpha; 
\beta)\), 
% il numero reale~\(|\alpha|\) rappresenta la
% misura della distanza del punto~\(P\) dall'asse~\(y\) e il numero 
reale~\(|\beta|\) 
...
% punti~\(P(+1;-3)\), \(Q(+5;+5)\), \(R(-2;+3)\), \(S(-5;-1)\) 
(figura~\ref{fig:D.12}).
% 
% \emph{Dati}:~\(P(+1;-3)\).
% 
% \emph{Obiettivo}:~\(PH\perp\) asse~\(x\), il segmento~\(PH\) è la distanza 
di~\(P\) 
% dall'asse~\(x\);
% \(PK\perp\) asse~\(y\), il segmento~\(PK\) è la distanza di~\(P\) 
dall'asse~\(y\).
% 
...
% monometrico~\(O_{xy}\), conoscendo le coordinate degli estremi~\(A\) 
e~\(B\) 
del 
% segmento stesso.
% 
...
%  \end{minipage}
%  \end{figure}
% \paragraph{Caso I} i due punti hanno la stessa ascissa. Il segmento~\(AB\) 
è 
...
% \emph{Procedura 
% 
risolutiva}:~\(\overline{AB}=\overline{AF}+\overline{BF}=y_{A}+(-y_{B})=y_{A}-
y_
{
% B}=5-(-3)=8\).
% 
...
% \emph{Procedura 
% 
risolutiva}:~\(\overline{AB}=\overline{BK}-\overline{AK}=-(y_{B})-(-y_{A})=y_{
A}
% -y_{B}=-1+6=5\).
% 
%  \end{esempio}
...
% la \emph{misura del segmento} \(AB\) \emph{parallelo all'asse delle 
ordinate} 
% è~\(\overline{AB}=|x_{a}-x_{B}|\)
% indipendentemente da quale estremo abbia ordinata maggiore.
% 
% \paragraph{Caso II} i due punti hanno la stessa ordinata. Il 
segmento~\(AB\) 
...
% nella figura accanto, verifica che in ogni 
caso~\(\overline{AB}=|x_{A}-x_{B}|\).
% 
% La \emph{misura del segmento} \(AB\) \emph{parallelo all'asse delle 
ascisse} 
% è~\(\overline{AB}=|x_{A}-x_{B}|\)
...
% 
% \paragraph{Caso III} è questo il caso generale: il segmento ha una 
direzione 
...
% \emph{Obiettivo}:~\(\overline{AB}\).
% 
% \emph{Procedura risolutiva}: tracciando da~\(A\) la parallela 
all'asse~\(x\) 
e 
% da~\(B\) la parallela all'asse~\(y\)
% si determina il vertice~\(C\) del triangolo rettangolo~\(ABC\) di 
cui~\(AB\) 
è 
% l'ipotenusa.
% Per il teorema di Pitagora si 
ottiene:~\(\overline{AB}=\sqrt{\overline{AC}^2 
+ 
...
% \).
% 
% La \emph{misura del segmento} \(AB\), \emph{note le coordinate} dei suoi 
estremi 
% è:
% 
% 
...
% \ref{ese:D.22}, \ref{ese:D.23}, \ref{ese:D.24}, \ref{ese:D.25}, 
\ref{ese:D.26},
...
%  \begin{esempio}
% \label{ex:D.18}
%  Conoscendo le coordinate degli estremi~\(A\) e~\(B\) di un segmento 
determiniamo
...
% Con ragionamento analogo tracciando dai punti~\(A\), \(B\), \(M\) le 
parallele 
...
% estremi~\(A\left(-{\frac{3}{4}};1\right)\), 
\(B\left(2;-\frac{1}{2}\right)\).
% 
% \emph{Dati}:~\(A\left(-{\frac{3}{4}};1\right)\), 
\(B\left(2;-\frac{1}{2}\right)\), 
% \(AM \equiv MB\).
% 
% \emph{Obiettivo}:~\(M(x_{M};y_{M})\).
...
%%%%%%%% testo che proviene dal capitolo sui razionali %%%%%%%%%%
...
Se si indica con~\(l\) la lunghezza del lato del triangolo, allora il 
perimetro 
è dato dalla relazione:~\[2p=3l.\]

È possibile notare che se raddoppia il lato, raddoppia anche il perimetro; 
se si triplica il lato, allora triplica anche il perimetro etc.

\begin{center}
 \begin{tabular*}{.9\textwidth}{l@{\extracolsep{\fill}}*{6}{c}}
\toprule
...
\begin{definizione}
  Due grandezze~\(x\) e~\(y\) si dicono \emph{direttamente proporzionali} se 
il 
  loro rapporto è costante, cioè \[\frac{y}{x}=k,\text{ con }k\neq~0.\]
\end{definizione}
...
passa per l'origine di un sistema di assi cartesiani ortogonali 
(figura~\ref{fig:3.2}).

\begin{inaccessibleblock}[Figura: TODO]
 \begin{figure}[h]
\begin{minipage}[t]{.48\textwidth}
  \centering\input{\folder lbr/fig035_prodir.pgf}
  \caption{Proporzionalità diretta.}\label{fig:3.2}  
\end{minipage}\hfil
 \begin{minipage}[t]{.48\textwidth}  
 \centering\input{\folder lbr/fig036_propinv.pgf}
 \caption{Proporzionalità inversa.}\label{fig:3.3}
 \end{minipage}
\end{figure}
\end{inaccessibleblock}

Esaminiamo ora un altro esempio. Se quando vai a fare benzina allo scooter 
chiedi ogni volta \officialeuro\ 10 di benzina, noterai che se aumenta il 
prezzo della benzina diminuirà la quantità di carburante che ricevi e 
viceversa se diminuisce il prezzo aumenterà la quantità di carburante che 
ricevi. Ciò che rimane costante è il prodotto tra il prezzo della benzina e 
la quantità di benzina ricevuta che deve essere sempre \officialeuro\ 10.

\begin{center}
 \begin{tabular*}{.9\textwidth}{l@{\extracolsep{\fill}}*{4}{c}}
\toprule
Prezzo benzina al litro~\(p\) (\officialeuro\ ) &1,126 &1,156 &1,212 &1,248\\
Benzina ricevuta~\(b\) (\unit{l}) &8,881 &8,650 &8,251 &8,013\\
Costo~\(c=p\cdot b \) (\officialeuro\ ) &10,00 &10,00 &10,00 &10,00\\
\bottomrule
\end{tabular*}
\end{center}

\begin{definizione}
  Due grandezze~\(x\) e~\(y\) si dicono \emph{inversamente proporzionali} se 
il 
...
Graficamente un tale tipo di proporzionalità è rappresentato da un ramo 
d'iperbole equilatera in un sistema di assi cartesiani ortogonali 
(figura~\ref{fig:3.3}).
\end{comment}
...
\paragraph{Caso~\(k>0\)} Quando ci proponiamo di costruire una tabella di 
valori, 
...
\paragraph{Significato dei coefficienti~\(m\) e~\(q\) nella funzione 
lineare~\(y = 
mx+q\)}
...
informazioni sull'angolo che la retta
 ...
 \item se~\(m>0\) l'angolo formato con l'asse delle ascisse è un angolo 
acuto; 
se~\(m<0\) l'angolo è ottuso;
 \item se~\(q=0\) la funzione è~\(y=ax\), il suo grafico è una retta passante 
per 
l'origine;
 \item se~\(q\neq~0\) esso è l'ordinata del punto di intersezione della retta 
con 
...
\conclusione la funzione costante e la funzione di proporzionalità diretta 
sono 
funzioni lineari.
...
=\).
\begin{itemize*}
\item nella formula della funzione avente~\(r\) come grafico si ha~\(m 
\ldots~0\) 
e~\(q \ldots~0\)
\item nella formula della funzione avente~\(s\) come grafico si ha~\(m 
\ldots~0\) 
e~\(q \ldots~0\)
\item nella formula della funzione avente~\(a\) come grafico si ha~\(m 
\ldots~0\) 
e~\(q \ldots~0\)
\item nella formula della funzione avente~\(b\) come grafico si ha~\(m 
\ldots~0\) 
e~\(q \ldots~0\).
\end{itemize*}
 \end{esempio}
% \end{exrig}
Assegnata una tabella di corrispondenza è possibile determinare la formula 
della 
funzione lineare.
% \begin{exrig}
 \begin{esempio}
Stabilisci se la tabella assegnata rappresenta una funzione lineare e 
determina 
...
proporzionalità diretta (il rapporto~\(y/x\) non è costante!). Per 
determinare 
la 
formula devo stabilire
il valore di~\(m\) (coefficiente angolare) e di~\(q\).
Dalla tabella individuo il valore~\(q=-2\), infatti per~\(x=0\) si 
ha~\(y=-2\). 
Per 
determinare~\(m\), sommo
\(2\) a tutte le ordinate e trovo la tabella della proporzionalità 
diretta~\(y=3x\).
\begin{center}
 \begin{tabular}{cccccc}
 \toprule
...
È assegnata la tabella che esprime il legame tra due variabili reali; 
determina 
...
suo quadrato.
Il dominio di tale funzione è~\(\Dom=\R\), mentre l'immagine 
è~\(\IM=\R^{+}\cup \{0\}\).
...
% \ref{ese:D.56}, \ref{ese:D.57}, \ref{ese:D.58}, \ref{ese:D.59}, 
\ref{ese:D.60}}
...
\begin{soluzione}
Pensando il peso come variabile indipendente che possa assumere qualunque 
valore 
reale positivo, possiamo rappresentare la
tabella esposta con un grafico~(figura~\ref{fig:D.29}).

Osserviamo che il punto~\(C\) rappresenta il costo di un pacco 
di~\(8\unit{Kg}\) il 
...
Particolare importanza assume la funzione valore assoluto definita da~\(\R\) 
a~\(\R\):
\[
...
Vogliamo tracciarne il grafico. Nel riferimento cartesiano ortogonale 
tracciamo 
la retta~\(y=x\) e su di essa evidenziamo
la semiretta~\(b\) avente l'origine in~\(O\) i cui punti appartengono al 
primo 
quadrante; analogamente tracciamo la retta~\(y=-x\)
e su di essa evidenziamo la semiretta~\(a\) avente l'origine in~\(O\) i cui 
punti 
...
\conclusione il grafico della funzione valore assoluto di equazione~\(y=|x|\) 
è 
formato da due semirette aventi come origine
l'origine del riferimento cartesiano. La funzione è continua, è nulla 
per~\(x=0\) 
e positiva per ogni~\(x\in \R-\{0\}\),
il codominio è~\(\Cod=\{y\in \R/\, y\ge~0\}\).
...
% \(\Dom_{f}=\Dom_{f_1}\cup \Dom_{f_2}=\R\).
% 
...
% 
% \paragraph{Passo III} tracciamo il grafico~(figura~\ref{fig:D.30}) che 
risulta 
...
% y=1\text{ se }x>0\\
% y=0\text{ se }x=0\\
% y=-1\text{ se }x<0\end{array}\right.\) e calcolare l'ordinata dei suoi 
punti~\(A\) 
% e~\(B\) sapendo che~\(x_{A}=34\) e~\(x_{B}=-5\).
%  \end{esempio}
% % \end{exrig}
% 
% \osservazione I grafici dei due esempi precedenti hanno una notevole 
differenza:
...
% le semirette del secondo esempio hanno invece origine diversa e il grafico 
non 
