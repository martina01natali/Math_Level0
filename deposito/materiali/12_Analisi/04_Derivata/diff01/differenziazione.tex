...
$m=\frac{\Delta y}{\Delta x}$ è sempre lo stesso, come è giusto che sia per una retta.
Da $x_B-x_A=\Delta x$ ricaviamo banalmente $x_B=x_A+\Delta x$, cioè nel piano cartesiano
...
\begin{definizione}
  Si dice \emph{Rapporto Incrementale} il rapporto degli
  incrementi, cioé la quantità~$\frac{\Delta y}{\Delta x}$.
\end{definizione}
Si tratta di una quantità finita, calcolabile se $\Delta x \ne 0$.\\
Il Rapporto Incrementale, calcolato su una retta fornisce la sua pendenza ed è un 
valore costante, come abbiamo visto.\\
Ma il calcolo si può applicare a qualsiasi funzione, anche a quelle che nel piano
cartesiano sono rappresentate da curve. In questo caso le cose cambiano.\\
...
\caption{Rapporti incrementali in una curva.} 
\label{}
\end{figure}
...
delle tangenti nell'altro. Seguono i calcoli.
$$  m_{AB}=-2 \qquad m_{BC}=\frac{1}{2}\qquad m_{CD}=\frac{1}{3}\qquad  m_{DE}=-\frac{7}{3}$$
\end{esempio}

Se il grafico non è una retta, ma una curva, il Rapporto Incrementale, 
calcolato fra varie coppie di punti, ha valori diversi!
Poiché $\Delta y$ non è costante, anche se si imposta $\Delta x$ costante, il R.I. 
cambia a seconda della coppia di punti fissati sulla curva. La cosa è ben visibile 
se si traccia la retta che unisce i due punti. Risulta allora evidente la proprietà
generale che segue.

\begin{osservazione}
 Il Rapporto incrementale fra le coppie di valori di una funzione è a sua volta una
 funzione che dipende dalla coppia scelta. \\
 Nel piano cartesiano, il Rapporto Incrementale calcola la pendenza della retta 
 secante che unisce due punti del grafico.\\
 Il R.I. esiste solo per $\Delta x\ne 0$.
\end{osservazione}
...
\begin{esempio}
Grafico di una curva con le secanti per AP4, AP3, AP2, Ap1 
Le secanti sono tutte relative al punto $A$. Man mano che $P$ si avvicina ad 
$A$, la secante tende ad allinearsi alla tangente ideale. 
\end{esempio}

Il secondo punto non deve arrivare a coincidere con $A$, altrimenti il R.I.
diventa incalcolabile\\
Quando $P$ è così vicino ad $A$ che la loro distanza è $\overline{AP}<\frac{1}{n}$,
$\forall n$, siamo nel campo degli infinitesimi: cambia la natura del rapporto che
avevamo imparato a calcolare. Trattandosi di un rapporto fra infinitesimi, non
possiamo essere certi che fornisca un risultato e di quale natura sia.

\begin{definizione}
 Si dice Rapporto Differenziale della funzione $f(x)$, relativo a $x_0$
 il rapporto $\frac{df(x)}{dx}|_{x=x_0}$ fra il differenziale della funzione 
 e quello della variabile, calcolati nel punto $x_0$. 
\end{definizione}
