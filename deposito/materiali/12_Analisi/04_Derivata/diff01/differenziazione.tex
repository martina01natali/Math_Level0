...
Dato che gli infinitesimi che vogliamo trascurare hanno origine già nel rapporto 
differenziale, chiediamoci:\\ 
\emph{Possiamo trascurare, cioè fingere che valgano zero, gli infinitesimi del 
rapporto differenziale, anche se abbiamo specificato che deve essere 
$dx \ne 0$?}\\
Sappiamo già che esiste il modo corretto: \emph{la funzione parte standard $\pst{}$, 
applicata a un iperreale finito, isola il numero reale infinitamente vicino
e permette così di ignorare i suoi infinitesimi}.
...
\end{cases}\)\\
 Differenziale per $x=0$: $df(0)=f(0+dx)-f(0)=0+dx-0=dx$;\\
 Rapporto differenziale $\dfrac{df(0)}{dx}=\dfrac{dx}{dx}=
 \begin{cases}
 1\quad &\mbox{per }x\ge 0\\
-1\quad &\mbox{ per }x<0
\end{cases}\)\\
 Derivata: non esiste per $x=0$, perché il rapporto differenziale dipende dal 
 segno di $x$. Infatti per $x<o$ ci si avvicina a $x=o$ da sinistra, cioè
 si danno a $x=0$ incrementi $dx$ negativi. I valori corrispondenti della 
 funzione $f(x)$ sono invece positivi.
\end{esempio}
...
% $df(x)$ assume lunghezza infinita e non può essere valutato. Nel punto 
% $\punto{0}{1}$ la tangente (linea tratteggiata) è verticale.
% \end{minipage}
% \end{center}
% \end{inaccessibleblock}
\label{}
...
$f'(4)$
 e $f'(2)$.\\
 La funzione contiene un valore assoluto e la si riscrive come  come se fosse 
 divisa in due rami:
...
quindi, anche seza derivare, avremo:
...
grafico e il rapporto risulta negativo. Al contrario, se $dx<0$ siamo nel ramo
sinistro e il rapporto è positivo: la parte standard del rapporto 
differenziale relativa al punto in cui $x=2$ non è unica.
...
Allora la funzione di partenza è equivalente a $f(x)=\sqrt{x}$? (Ad essere precisi, 
...
\begin{esempio}
  ...
  Soluzione: $v'(u)=\dfrac{1}{8}2u=\dfrac{u}{4}$. Infatti il differenziale è $dv(u)=\dfrac{1}{8}[2udu+(du)^2]$ perché $df(x)=2xdx+(dx)^2$ 
...
inversa. Ecco alcuni esempi di semplici funzioni algebriche, dirette e inverse,
con le rispettive derivate..
\begin{center}
...
della parabola, perché altrimenti la formula inversa non potrebbe essere una 
funzione. Occorre sempre porre attenzione al dominio di $f$, quando si vuole 
definirne l'inversa. Una buona regola pratica per capire se $f^{-1}$ è 
...
% \end{proof}
\begin{osservazione}
 Anche questo teorema necessita di un'introduzione simile a quella che precede 
 l'enunciato sulla derivata della funzione composta. Perciò rimandiamo a quelle parole, evitando la dimostrazione.
\end{osservazione}
\begin{esempio}
...
Abbiamo già discusso (pag.\pageref{subsubsec:insnum_expstar} il comportamento
di queste funzioni nell'insieme degli Iperreali e abbiamo ormai un bagaglio
di conoscenze sulle derivate che agevola il lavoro.
...
Per  derivare, ora dovremmo applicare la parte standard e se vogliamo seguire
le indicazioni del grafico, il risultato dovrà essere un'esponenziale. Quindi
dobbiamo concentrarci sul rapporto $\dfrac{\tonda{a^{dx}-1}}{dx}$, che 
nell'espressione precedente è il fattore che potrebbe causare un grafico della 
derivata diverso da quello esponenziale che abbiamo intuito.\\
La funzione esponenziale generica $f(x)=a^x$ ha proprietà caratteristiche:\\
$f(0)=a^0=1$ e $f(x+y)=a^{x+y}~=~a^x\cdot a^y$.\\
Quindi: $f(0)=1$ e $f(x+dx)=f(x)f(dx)$. Così il rapporto differenziale in 
queste funzioni diventa:\\
$\dfrac{df(x)}{dx}=\dfrac{f(x+dx)-f(x)}{dx}=\dfrac{f(x)f(dx)-f(x)}{dx}=
\dfrac{f(dx)-1}{dx}f(x)=\dfrac{f(0+dx)-f(0)}{dx}f(x)$.\\
L'ultima espressione contiene il rapporto differenziale calcolato in $x=0$,
quindi $\dfrac{df(x)}{dx}\sim f'(0)\cdot f(x)$.\\
Allora per la derivata, si ha:\\
$f'(x)= f'(0)\cdot f(x)$.\\
La derivata di una funzione esponenziale è proporzionale alla
funzione stessa, attraverso un fattore che corrisponde alla derivata 
calcolata in $x=0$.\\
Per capire meglio l'effetto di questo fattore, fingiamo per un attimo che non
esista, cioè che nell'espressione valga $1$. In questo modo la funzione e la
sua derivata sarebbero proprio identiche e i due grafici sarebbero sovrapposti:\\
