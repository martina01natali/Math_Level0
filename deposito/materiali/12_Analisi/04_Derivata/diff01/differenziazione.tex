...
\begin{itemize}[nosep, noitemsep]
 ...
 \item derivabile in un sottoinsieme \(S\) del dominio di \(f\), se esiste 
\(f'(x)\) per ogni \(x\in S\);
 \item derivabile per ogni \(x\) del dominio di \(f\), nel qual caso si dice 
brevemente che ``\(f\) è derivabile''
\end{itemize}
...
% sarebbe un teorema, ma mi sembra di appesantire. va bene come 
% osservazione?(B)
...
% Ho spostato il teorema a dopo la regola del rapporto (B)
...
\(f(x)=\dfrac{1}{x^3}\), e così via, con \(x\ne 0\). Al termine di un certo 
numero di calcoli ci troveremmo a confermare la regola già vista per 
\(f(x)=x^n\). Ma ci arriveremo più comodamente grazie al successivo 
teorema~\ref{teo:diff01_deri_x_alla_z}.
...
% Tesi: \(f'(x)=zx^{z-1}\).
\begin{proof}
 Se \(z\geqslant 0\), corrisponde a un numero naturale e quindi la regola è già 
stata dimostrata. Se invece \(z<0\), basta applicare la regola del rapporto, 
come nell'esempio~\ref{esempio:derivata_x_alla_z}.
\end{proof}
