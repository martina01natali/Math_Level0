...
Sappiamo già che dovrà risultare zero. \quad 
...
\begin{itemize} [nosep]
\item Se \(f'(x_P)\) è definito la tangente a \(f\) in \(P\) è: \quad 
\(y = f'(x_P)\tonda{x - x_P} +y_P\)
\item se il Rapporto Differenziale è un infinito è: \quad 
\(x = x_P\)
\end{itemize}
\end{definizione}
...
attraverso il differenziale e il Rapporto Differenziale che spesso sono calcoli 
assai complicati.
...
Possiamo concludere è che \emph{a distanza \(dx\) dal punto che si vuole 
...
Il campo visivo di \(M_1\) riesce a separare \(x_0\) e \(x_0 +dx\). 
...
\item la composizione di una funzione con lasua inversa dà per risultato la 
