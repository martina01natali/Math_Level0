...
\begin{osservazione}
 Ripetendo l'osservazione a pag.\pageref{subsubsec:diff01_diffpot} relativa a questo
...
GRAFICI AFFIANCATI f(x)=exp e f'

Si può dire che anche l'andamento delle pendenze delle tangenti, cioè
l'andamento della derivata della funzione, è anch'esso esponenziale.
...
DISEGNO GRAFICI ESP E LOG SIMMETRICI 

\begin{teorema}
  La derivata della funzione logaritmo naturale è: $\mathit{D}\quadra{\ln x}=
  \dfrac{1}{x}$.
\end{teorema}
\noindent Ipotesi: $f(x)=\ln x$. \tab $f'(x)=\dfrac{1}{x}$
\begin{proof}
...
l'angolo sono indistinguibili: $\st\tonda{\frac{\sin \epsilon}{\epsilon}}=1$.
...
\end{osservazione}
\begin{osservazione}
GRAFICO F(X)=SEN X  con tangenti e grafico derivata affiancati 
Anche il grafico dell'andamento delle tangenti può confermare la tesi
...
\begin{proof}
Il disegno che dimostra la tesi è uguale al precedente e ti invitiamo a
riprodurlo. Questa volta l'attenzione sarà
...
GRAFICO F(X)=cos X  con tangenti e grafico derivata affiancati
\begin{esempio}
...
GRAFICO F(X)=tang X  con tangenti e grafico derivata affiancati

\begin{teorema}
   La derivata della funzione $f(x)=\tg x$ è $\mathit{D}\quadra{\tg x}=
   \dfrac{1}{\cos^2 x}=1+tg^2 x$ per $x= \pm\frac{\pi}{2}$.
\end{teorema}
...
Per $x\approx\dfrac{\pi}{2}$ la tangente ha un grafico che cresce verticalmente,
...
$k$ & 0 &   $\tg x$ & $\dfrac{1}{cos^2 x}=\tg^2 x +1$\\ \hline
$x$ & 1 &   $\log_a x$ & $\dfrac{1}{x\ln a}$\\ \hline
$x^\alpha$ & $\alpha x^{\alpha-1}$ &   $\ln x$ & $\dfrac{1}{x}$\\ \hline
$\sen x$ & $\cos x$ &   $a^x$ & $a^x\ln a$\\ \hline
$\cos x$ & $-\sen x$ &   $e^x$ & $e^x$\\ \hline
\end{tabular}
\end{center}
}%
\end{minipage} 
...
\mc{2}{c}{Regole di derivazione}\\\hline
\mc{2}{|c|}{$\mathit{D}\quadra{f(x)+g(x)}=f'(x)+g'(x)$}\\\hline
\mc{2}{|c|}{$\mathit{D}\quadra{kf(x)}=kf'(x)$}\\\hline
\mc{2}{|c|}{$\mathit{D}\quadra{f(x)\cdot g(x)}=f'(x)\cdot g(x)+f(x)\cdot g'(x)$}\\\hline
\mc{2}{|c|}{$\mathit{D}\quadra{\dfrac{1}{f(x)}}=-\dfrac{f'(x)}{f^2(x)}$}\\\hline
\mc{2}{|c|}{$\mathit{D}\quadra{\dfrac{f(x)}{g(x)}}=\dfrac{f'(x)\cdot g(x)-f(x)
    \cdot g'(x)}{g^2(x)}$}\\\hline
\mc{2}{|c|}{$\mathit{D}\quadra{f(g(x))=f'(g(x))\cdot g'(x)}$}\\\hline
\end{tabular}
\end{center}
}%  
\end{minipage} 
...
anche nello studio delle scienze umane e sociali. Al momento, i problemi che 
siamo in grado di affrontare ruotano attorno al concetto di tasso di variazione:
data una funzione, si deve cercare quanto rapidamente essa varia rispetto
alla sua variabile. La derivata è il calcolo che consente di affrontare
il problema.\\
In casi un po' più articolati è utile anche l'equazione alle differenze 
nella forma diretta $\Delta f(x)\cong f'(x_0)\Delta x$, o anche nella forma
inversa $\Delta x \cong \dfrac{\Delta f(x)}{f'(x_0)}$. La sua utilità viene dal
fatto che si tratta di un'equazione di primo grado in $\Delta x$, perché i 
termini infinitesimi di grado superiore sono trascurati. Si ottengono così
delle soluzioni approssimate, ma in genere il grado di imprecisione è 
sopportabile.
...
\begin{esempio}
 ...
 \begin{osservazione}
 Si tratta di un problema tipico sull crescita esponenziale, di quelli già 
 risolti quando ancora non conoscevi l'esistenza delle derivate. Come mai in
 un caso del genere l'uso delle derivate non è indispensabile? Perché
...
Il costo marginale, cioè per unità in più, è quindi dello $0,2\%$.
\begin{osservazione}
