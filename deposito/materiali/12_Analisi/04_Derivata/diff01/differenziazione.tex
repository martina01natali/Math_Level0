...
parametri, possiamo eseguirla variando il loro valori e otterremo nel 
primo caso~$6$, nel secondo~$2$ e nel terzo~$0$. Ovviamente possiamo 
variare anche la funzione ottenendo dei valori completamente diversi. 
...
Nelle funzioni \emph{continue}, incrementi infinitesimi della~$x$ producono 
incrementi infinitesimi della~$y$.
Questi incrementi sono osservabili solo utilizzando un microscopio non 
standard.
...
qualunque sia il valore nel quale si calcola il differenziale si ottiene un 
valore infinitesimo queste funzioni sono quindi continue in tutto \(\IR\).
...
capire che se no è definita la funzione non può essere definito neppure il 
differenziale.
...
Recuperiamo il differenziale esatto della funzione:
\[d\tonda{\sqrt(0)}=\frac{dx}{\sqrt{0+dx}+\sqrt{0}}\]
...
\[d\tonda{\sqrt(0)}=\frac{dx}{\sqrt{dx}}\]
Razionalizzando il denominatore della frazione, il differenziale diventa:
\[d\tonda{\sqrt(0)}=\frac{dx}{\sqrt{dx}} \cdot \frac{\sqrt{dx}}{\sqrt{dx}}
  \sqrt{dx}\]
Ma \(\sqrt{dx}\) è un infinitesimo ''molto`` più grande di \(dx\). 
...
% 
% TODO chiedere: 1/0 non è definito neppure in *R quindi non c'è telescopio 
% che tenga...
% Nel piano iperreale, puntando l'infinito con un telescopio,
% non riusciamo a focalizzare la differenza infinitesima fra due valori 
% infiniti della funzione.
% 
...
riconoscere dei punti di discontinuità di una funzione, non degli insiemi di 
discontinuità.
