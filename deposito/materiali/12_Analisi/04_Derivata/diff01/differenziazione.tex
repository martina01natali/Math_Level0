...
% - - funzione derivata
% - Teoremi
% - - funzioni elementari algebriche
% - - classi di funzioni
...
3 g\cdot dt+\frac{1}{2} g (dt)^2 %= g \cdot dt \tonda{3+ \frac{1}{2} dt}
\] 
...
Quindi: \quad \(f(0) = -0^3 -2\cdot 0^2 +0 +1 = 1\)\\
e la tangente è: \quad \(y = +1 \tonda{x - 0} +1 \srarrow y = x +1\)
}
\osservazione
...
\begin{esempio}
% Calcola la pendenza in \(x_0 = -1\) della funzione \(f(x)=\sqrt[3]{3x}\)
...
%               {\epsilon}} = 
%    \pst{\dfrac{-4\epsilon +\epsilon^2}{\epsilon}} = 
%    \pst{\dfrac {\cancel{\epsilon}\tonda{-4 +\epsilon^2}}
%                {\cancel{\epsilon}}} = -4 
% \\
% f'(-5) &= \dots
% \end{align*}
...
\begin{center} \scalebox{.7}{\parabolaederivata} \end{center}
\end{inaccessibleblock}
\label{graf:funzioneederivata}
}
Possiamo formulare la congettura che la derivata della funzione 
\(f(x)=\dfrac{1}{2}x^2+2x+4\) \quad sia \quad 
\(f'(x) = x +2\).
...
Calcola la derivata della funzione \\ \(f(x)=5x\).

Differenziale: \\
\(df(x) = 5\tonda{x +\epsilon} - 5x = 5x +5\epsilon - 5x = 5\epsilon\) 

Rapporto Differenziale: \\
\(RD = \dfrac{df}{\epsilon} = \dfrac{5\epsilon}{\epsilon} = 5\)
...
Differenziale: 
\begin{align*}
df(x) &= \dfrac{\tonda{x +\epsilon}^4}{4} + \tonda{x +\epsilon} -
          \dfrac{x^4}{4} -x= \\
&= \dfrac{x^4 +4x^3\epsilon+6x^2\epsilon^2+4x\epsilon^3+\epsilon^4}{4} + x 
   +\epsilon -\dfrac{x^4}{4} -x= \\
&= \dfrac{+4x^3\epsilon+6x^2\epsilon^2+4x\epsilon^3+\epsilon^4}{4} +\epsilon 
\end{align*}
Rapporto Differenziale: 
}{
\scalebox{1}{\esempioquartica}
}
\begin{align*}
RD &= \dfrac{df}{\epsilon} = 
...
In modo simile si può calcolare la seguente derivata.
\begin{esempio}
...
\hspace*{-10mm}\affiancati{.49}{.49}{
\begin{center} \scalebox{1}{\radquad} \end{center}
}{
...
\affiancati{.38}{.58}{
\vspace{15mm} 
\scalebox{.9}{\incrementaleprodotto}
}{ 
\vspace{-10mm} 
\scalebox{.9}{\differenzialeprodotto}}
\end{inaccessibleblock}
...
% La somma è quindi indistinguibile dalla somma dei primi due termini:
...
\begin{esempio}
% Il teorema del prodotto di funzioni ci permette di dare un'ulteriore 
% dimostrazione della derivata della potenza con esponente naturale.
%
...
% la sua derivata si ottiene calcolando
...
Possiamo concludere che \emph{a distanza \(dx\) dal punto che si vuole 
esaminare, fra la curva e la tangente c'è una differenza pari a 
\(\delta\cdot dx\), un infinitesimo di ordine superiore a \(dx\)}. 
...
\begin{inaccessibleblock}
  [differenziale della tangente]
    \begin{center} \scalebox{.9}{\differenzialenst} \end{center}
\end{inaccessibleblock}
}{
Nel punto \(\punto{x_0}{f(x_0)}\) il grafico della funzione e la tangente
sono indistinguibili. 

Per capire meglio la situazione, utilizziamo alcuni microscopi non standard 
con ingrandimenti infiniti.

L'ingrandimento infinito di \(M_1\) riesce a separare \(x_0\) e \(x_0 +dx\). 
A questo livello microscopico la curvatura del grafico non esiste, 
per cui la curva e la tangente sono sovrapposte. 

Nemmeno il microscopio non standard \(M_2\) puntato, nel primo microscopio, 
su \(\punto{x_0}{f(x_0)}\), per cui l'ingrandimento complessivo diventa 
\(\infty^2\), può distinguere i due grafici. 

Ma un microscopio non standard \(M_3\), puntato nel primo microscopio a 
distanza \(dx\) da \(x_0\), mostra la tangente e 
la curva come rette parallele, che distano fra loro 
\(\delta \cdot dx\), cioè un infinitesimo di un infinitesimo.
% Nel punto \(\punto{x_0}{f(x_0)}\) il grafico della funzione e la tangente
% sono indistinguibili. 
...
% \begin{center} \scalebox{.9}{\differenzialest} \end{center}
\scalebox{.9}{\differenzialest} 
\end{inaccessibleblock}
}{
...
\begin{align*}
f'(x) &= f'(g(x)) \cdot g'(x) = \\
&= \pst{\dfrac{1}{2\sqrt{25-x^2}} \cdot \tonda{-2x}} = 
\dfrac{-x}{\sqrt{25-x^2}}
\end{align*}
...
\affiancati{.52}{.46}{
% \vspace*{-40mm}
Poniamo \(g(x)=x^2 -x -2\)\quad e\quad \(f(g(x))=\sqrt{g(x)}\). 
Allora: 
\begin{align*}
...
\end{align*}
La pendenza in \(P\) è: \quad 
\(f'(3) = \dfrac{2 \cdot 3 -1}{2 \sqrt{3^2 -3 -2}}= \dfrac{5}{4}\) 
}{
% \vspace*{-20mm}
% \hspace{-30mm} 
\scalebox{1}{\derivacompostab}
}

% \vspace*{-40mm}
e la tangente avrà equazione: \quad 
\(y = \dfrac{5}{4} \tonda{x -3} +2 \srarrow 
  y = \dfrac{5}{4} x -\dfrac{7}{4}\)
\end{esempio}

\begin{esempio}
Calcola la derivata di: \quad \(f(x)=\dfrac{1}{\sqrt{2x-3}}\).
...
&= -\dfrac{1}{\tonda{2x-3} \cdot \sqrt{2x-3}} = 
   - \tonda{2x-3}^{-\frac{3}{2}}
\end{align*}
}{
...
\(x = \dfrac{6}{y} -2\) \quad che è equivalente a 
\quad \(y = \dfrac{6}{x +2}\) \\
...
% %%% La seguente affermazione è smentita dall'esempio precedente.
% %%% Altro controesempio arcsin
% %%% Mentre in arctan l'ID è più ampio che in tan.
...
% \begin{osservazione}
% Si vede che il grafico di \(f\) e il grafico 
% della sua inversa \(f^{-1}\) sono simmetrici rispetto alla bisettrice del primo 
% e terzo quadrante: \(y=x\). Infatti questa simmetria si ottiene proprio 
% scambiando \(x\) con \(y\). 
% \end{osservazione}
% 
...
Possiamo pensare di generalizzare la conclusione dell'esempio precedente. 
...
% dato che: \(\Deriv{f(g(x))} = f'(g) \cdot g'(x) = 1\), allora:
% \[\Deriv{f(f^{-1}(x))} = \Deriv{f(f^{-1})} \cdot \Deriv{f^{-1}(x)} = 1 
% \srarrow \Deriv{f^{-1}(x)} = \dfrac{1}{\Deriv{f(f^{-1})}}\]
% % \[\Deriv{f(f^{-1}(x))} = \Deriv{f(f^{-1})} \cdot \Deriv{f^{-1}(x)} = 1 \srarrow
% % \Deriv{f^{-1}(x)} = \dfrac{1}{\Deriv{f(f^{-1})}}\]
...
% Ogni punto \(\punto{x}{f(x)}\) 
% sulla curva \(y=f(x)\) 
% ha un corrispondente \(\punto{y}{f^{-1}(y)}\) 
% sulla curva della funzione inversa, 
% nella simmetria rispetto alla bisettrice. 
% Guardiamo come si corrispondono i differenziali:
% \(dx\) e \(dy\) di una curva sono invertiti rispetto ai differenziali 
% dell'altra.
% Quindi la derivata di \(f(x)\) calcolata in \(x\) è il reciproco 
% della derivata di \(f^{-1}(x)\) calcolata in \(y\).
\end{minipage}
\end{center}
\end{inaccessibleblock}
\label{}
...
\affiancati{.34}{.64}{
\hspace{-5mm}\scalebox{1}{\dsincos}
% \hspace{-5mm}\scalebox{1}{\darcsin}
% \hspace{-5mm}\scalebox{1}{\darccos}
...
% \(\Deriv{\sen x} = \cos x\)
La derivata del seno di \(x\)~~è~~il coseno di \(x\): \quad 
\(\Deriv{\sen x} = \cos x\).
\end{teorema}
...
% presentati a pag. \pageref{subsubsec:insnum_fseno}.
\begin{proof}
\begin{align*}
...
\(\dfrac{1}{\cos^2 x} \stext{ o } 1+\tg^2 x\): % per \(x\ne \pm\frac{\pi}{2}\): 
...
L'incremento \(AB\) dell'arco \(x\) non è infinitesimo.
% Nel disegno è rappresentato un 
% incremento non infinitesimo dell'arco \(x\).
% Osserviamo che:
...
\item nel triangolo \(OHC\): l'ipotenusa \(OC\) è lunga 
\(\dfrac{1}{\cos x}\);
...
usare microscopi non standard. 
% Entrambi i microscopi della figura seguente hanno lo stesso ingrandimento 
% infinito.
...
%        \dfrac{1}{\cos^2 x} \]
Il Rapporto differenziale ottenuto è un numero reale e quindi è la derivata 
cercata.
\end{proof}
...
E per \(x=\dfrac{\pi}{2}\)?\\
\(f'(x)=1+tg^2 x\srarrow f'\tonda{\dfrac{\pi}{4}}=1+\tg^2\dfrac{\pi}{4}=2\)\\
\(f'(x)=1+tg^2 x\srarrow f'\tonda{\dfrac{\pi}{2}}=1+\tg^2\dfrac{\pi}{2}=\)???
\end {esempio}


\subsection{Derivata delle inverse  delle funzioni circolari}
\label{subsec:differenziazione_circolari_inverse}

% TODO: grafici delle funzioni con tangenti e della derivata.

\subsubsection{Derivata di \(f(x)=\arcsin x\)}

\affiancati{.39}{.59}{
\hspace{-5mm}\scalebox{1}{\darcsin}
% \hspace{-5mm}\scalebox{1}{\darccos}
\label{fig:differenziazione_darcsin}
}{
Alla scala naturale, l'incremento del seno, \(dx\), non è visibile.
Per poterlo vedere dobbiamo operare un ingrandimento infinito nel punto \(A\).

Nel microscopio non standard ora è visibile  il triangolo \(ABC\) che è 
simile al triangolo \(AOD\). 

\(DA\) è il seno dell'angolo e per il teorema di Pitagora \(OD\) vale:
\(\sqrt{1-x^2}\)
% 
% \(AB\) è il differenziale della funzione \(\arcsin x\).
\begin{itemize} [nosep]
\item \(AB\) è il differenziale della funzione \(\arcsin x\); 
\item \(CB\) è il differenziale del seno. 
\end{itemize}
}
...
\begin{inaccessibleblock}
  [Arcoseno con pendenze e sua derivata]
\hspace{-20mm}\affiancati{.55}{.43}{
\begin{center} \scalebox{.8}{\arcoseno} \end{center}
}{
\begin{center} \scalebox{.8}{\derivataarcoseno} \end{center}
}
\end{inaccessibleblock}
\label{img:diff_parabola_con_pendenze}
\begin{center} 
...
\affiancati{.39}{.59}{
\hspace{-5mm}\scalebox{1}{\darccos}
\label{fig:differenziazione_darccos}
}{
\begin{proof}
Questa volta
\begin{itemize} [nosep]
\item la variabile indipendente è \(OD\);
\item la variabile dipendente è l'arco \(HA\);
\item \(AB\) è il differenziale della funzione \(\arccos x\); 
\item \(CA\) è il differenziale del coseno. 
\end{itemize}
...
\hspace{5mm}
\(RD = \dfrac{d (\arccos x)}{dx} = \dfrac{AB}{AC} = 
       \dfrac{AO}{-AD} = 
       -\dfrac{1}{\sqrt{1-x^2}}\)
\end{proof}
}
...
\begin{inaccessibleblock}
  [arcocoseno con pendenze e sua derivata]
\hspace{-20mm}\affiancati{.55}{.43}{
\begin{center} \scalebox{.8}{\arcocoseno} \end{center}
}{
\begin{center} \scalebox{.8}{\derivataarcocoseno} \end{center}
}
\end{inaccessibleblock}
\label{img:diff_parabola_con_pendenze}
\begin{center} 
...
% \[d(\arctan x) = AB = 
%   \dfrac{CE}{\sqrt{1 +x^2}} = CE \dfrac{1}{\sqrt{1 +x^2}}\] 
...
\begin{inaccessibleblock}
  [arcotangente con pendenze e sua derivata]
% \hspace{-20mm}\affiancati{.55}{.43}{
...
\label{subsec:differenziazione_tangente}

Riprendiamo ora qualche esempio di esercizio con le tangenti, 
vedi definizione \ref{def:differenziazione_tangente}.

\affiancati{.49}{.49}{
\begin{esempio}
Trova le equazioni delle tangenti alla parabola \quad 
\(f(x) = -\dfrac{1}{4}x^2 -2x +3\) nei suoi punti di ascissa: \quad
\(-4; \quad -2; \quad 0; \quad +2\).

Calcoliamo la derivata della funzione: \quad 
\(f'(x) = -\dfrac{1}{2}x -2\)
...
\begin{center}
\begin{tabular}{rrrl}
\mc{1}{c}{\textit{\(x_0\)}} & \mc{1}{c}{\textit{\(f(x_0)\)}} & 
\mc{1}{c}{\textit{\(f'(x_0)\)}} & 
\mc{1}{c}{\textit{\(y = f'(x_0) \tonda{x -x_0} + f(x_0)\)}}\\
\toprule
\(-4\) & \(+7\) & \(0\) & \(y = +7\)\\
\(-2\) & \(+6\) & \(-1\) & \(y = -x +4\)\\
\(0\) & \(+3\) & \(-2\) & \(y = -2x +3\)\\
...
\begin{inaccessibleblock}
  [Parabola con alcune tangenti]
\begin{center} \scalebox{.8}{\parabolaapplicazionia} \end{center}
\end{inaccessibleblock}
}
...
alla curva\\ 
\(f(x) = -2x^3 +x\) \quad nel punto di ascissa \quad \(x_0 = -1\).

\affiancati{.39}{.59}{
\begin{inaccessibleblock}
  [Cubica e tangente in x=-1]
\begin{center} \scalebox{.8}{\cubicaapplicazioni} \end{center}
\end{inaccessibleblock}
}{
\noindent Calcolo del valore della funzione in \(-1\): \\ 
\(f(-1) = -2 \cdot \tonda{-1}^3+\tonda{-1} = +2 -1 = +1\)\\
Calcolo della pendenza in \(-1\): \\ 
\(f'(x) = -6x^2+1 \srarrow f'(-1) = -6 \cdot \tonda{-1}^2+1 = -5\) \\
Da cui l'equazione della tangente: \\ 
\(y = -5(x+1)+1 = -5x-5+1 = -5x-4\)\\
L'intersezione con l'asse \(y\): \\ 
\(x=0 \srarrow y=-4 \srarrow \punto{0}{-4}\)
con l'asse \(x\): \\ [5pt]
\(y=0 \srarrow 5x = -4 \srarrow \punto{-\dfrac{4}{5}}{0}\)
}
\end{esempio}

\begin{esempio}
In quale punto del suo grafico la parabola \(y=4x^2-3x+2\) è inclinata di 
\(45^\circ\)?
...
Dobbiamo quindi imporre alla derivata il valore \(1\): \(f'(x) = 1\).

\(\Deriv{4x^2-3x+2} = 8x-3 = 1 \srarrow x =\dfrac{1}{2}\)

e il punto è: \quad
\(\punto{\dfrac{1}{2}}{\dfrac{3}{2}}\)
}{
\begin{inaccessibleblock}
  [Cubica e tangente in x=-1]
\begin{center} \scalebox{.8}{\parabolaapplicazionib} \end{center}
\end{inaccessibleblock}
}
\end{esempio}
...
equilatera. \\
Quindi \(V = V\punto{2}{2}\),
perché la funzione è \(y=\dfrac{4}{x}\),\\ 
la sua derivata in \(V\) è \tab 
\(f'(2)= -\dfrac{4}{4^2}=-1\)\\
e l'equazione della tangente in \(V\) è \tab \(y=-1(x-2)+2=-x+4\)\\
La retta \(y=-x+4\) interseca gli assi in: \tab 
\(\punto{4}{0}\) e \(\punto{0}{4}\) \\
ed è facile verificare che il punto \(V\) è medio fra i due. \\
Per ragioni di simmetria accade lo stesso con il vertice opposto 
\(\punto{-2}{-2}\).
}{
\begin{inaccessibleblock}
  [Iperbole equilatera e tangente nel vertice]
\begin{center} \scalebox{.8}{\iperboleapplicazioni} \end{center}
\end{inaccessibleblock}
}
...
\affiancati{.49}{.49}{
Incontriamo un primo problema: non abbiamo una sola ordinata, ma due; 
infatti \(x^2+y^2=20\) non rappresenta una funzione. 
Scegliamo \(+2\) infatti possiamo già valutare a occhio che il segmento 
che unisce l'origine con il punto \(\punto{4}{-2}\) non può essere inclinato 
di \(30^\circ\) rispetto all'asse \(x\), forse \(330^\circ\) o \(-30^\circ\). 
L'inclinazione del segmento è data da 
\[\dfrac{\Delta y}{\Delta x} = \dfrac{y_P - y_O}{x_P - x_O} = 
  \dfrac{2}{4} = \dfrac{1}{2}\]
L'angolo si ottiene calcolando l'arcotangente della pendenza: 
\[\alpha = \arctan{\dfrac{1}{2}} \simeq 26,56505118\]
}{
\begin{inaccessibleblock}
  [Circonferenza con tangente e raggio nel punto di tangenza]
\begin{center} \scalebox{1}{\inclinazioneraggio} \end{center}
\end{inaccessibleblock}
}
...
% rac{-1}{m_t}\) è appunto 
% l'antireciproco della pendenza della
La sua derivata è:
...
Mentre solo per la circonferenza (e la sfera) possiamo parlare di raggio e di 
...
% la pendenza della normale \(m_n = \frac{-1}{m_t}\) è appunto 
...
% \[f'(1) = \dfrac{2x(\ln x-1)-(x^2-1)\dfrac{1}{x}}{(\ln x -1)^2}\bigg|_{x=1}
%         = \dfrac{2\cdot 1(0-1)-(1-1)\cdot 1}{(0-1)^2} = -2\]
\affiancati{.59}{.39}{
Pendenza della funzione nel punto:
\begin{align*}
f'(1) &= \dfrac{2x(\ln x-1)-(x^2-1)\dfrac{1}{x}}{(\ln x -1)^2}\bigg|_{x=1}=\\
      &= \dfrac{2\cdot 1(0-1)-(1-1)\cdot 1}{(0-1)^2} = -2
\end{align*}
Ordinata del punto: \quad 
\(y_P = \dfrac{1^2-1}{\ln 1 -1} = 0\)\\[4pt]
Equazione della tangente: \quad 
\(y = -2(x-1) = -2x+2\)\\
Equazione della normale: \quad 
\(y = \dfrac{1}{2}(x-1) = \dfrac{1}{2}x-\dfrac{1}{2}\).
}{
\begin{inaccessibleblock}
  [Iperbole equilatera e tangente nel vertice]
\scalebox{1}{\tangenteenormalea}
\end{inaccessibleblock}
}
\end{esempio}
...
\end{align*}
Ordinata del punto: \quad 
\(y_P = \dfrac{1^2+1}{\ln 1 +1}= \dfrac{1+1}{0 +1} = 2\)\\[4pt]
Equazione della tangente: \quad 
\(y = 0(x-1)  +2 = +2\)\\
Equazione della normale: \quad 
\(x = x_P \srarrow x = 1\).
...
\subsection{Curvatura di una linea}
...
Il segmento \(AB\) si ottiene applicando il teorema di Pitagora al triangolo 
\(ABC\): 
\[AB = \Delta s = \sqrt{\tonda{\Delta x}^2 + \tonda{\Delta x}^2}\] 
Passando a incrementi infinitesimi, 
...
che è \(dy \sim f'(x)dx\), ma per il principio di transfer le proprietà 
geometriche si mantengono:
% \begin{align*}
% ds &= \sqrt{(dx)^2+(dy)^2}\sim \\
%    &\sim \sqrt{(dx)^2+(f'(x)dx)^2}=dx\sqrt{1+(f'(x))^2}
% \end{align*}
}{
%TODO   disegno tipo p.396, magari in una semicirconf come un grafico già pronto
\begin{center}  \scalebox{1}{\curvaturads} \end{center}
}
\[ds = \sqrt{(dx)^2+(dy)^2} \sim 
       \sqrt{(dx)^2+(f'(x)dx)^2} = dx\sqrt{1+(f'(x))^2}\]
...
\affiancati{.55}{.43}{
\paragraph{Le direzioni:} la direzione lungo una curva è la sua la pendenza, 
che corrisponde al coefficiente angolare della tangente alla curva nel punto 
considerato. 

Conoscendo l'angolo \(\alpha\), la pendenza 
\(f'(x)\) in quel punto è \(f'(x)=\tg(\alpha)\) e quindi 
\(\alpha=\arctan(f'(x))\).
% Quanto vale nei reali, vale anche passando agli iperreali quindi, 
% dal teorema dell'incremento si ha:
Lo stesso vale anche per incrementi infinitesimi:\\
...
%  Valori calcolati da me:
% {0.5: 0.22826882356360753, 1: 0.7071067811865475, 4: 0.03106778454421443}
\begin{align*}
x_A&=0,5 \quad\longrightarrow \quad k(0,5)=\dfrac{2\cdot 
0,5^3}{\sqrt{(0,5^4+1)^3}}\simeq 0,228\\
x_B&=1 \quad  \longrightarrow \quad k(1)=\dfrac{2\cdot 
1^3}{\sqrt{(1^4+1)^3}}=\dfrac{\sqrt{2}}{2}\simeq 0,707\\
x_B&=4 \quad  \longrightarrow  \quad k(4)=\dfrac{2\cdot 
4^3}{\sqrt{(4^4+1)^3}}=\simeq 0,031
\end{align*}
}{
\begin{center}  \scalebox{1}{\curvaturaesea} \end{center}
}
% \pagebreak %--------------------------------------
...
\affiancati{.49}{.49}{
\begin{esempio}
Che curvatura ha una retta, per esempio \(y=3x-1\)\,? 

La risposta viene d'istinto, trattandosi di una retta. Calcoliamo:
\begin{align*}
...
retta, anche quelle verticali, anche se in questo caso non possiamo 
applicare la formula.
\end{esempio}
}{
\begin{center}  \scalebox{1}{\curvaturaeseb} \end{center}
}
...
\begin{center}  \scalebox{1}{\curvaturaesec} \end{center}
}
\end{esempio}
...
% \(r(0) = \dfrac{\quadra{1+(f'(x))^2}^{\frac{3}{2}}}{f''(x)} =
...
\affiancati{.39}{.59}{
Le coordinate del centro \(O_P\) del cerchio osculatore in \(P\) saranno 
quindi:
% \begin{align*}
...
% Penso che vada:
\begin{align*}
x_{O_P} &= x_P +r \cos\beta = \\
        &= x_P +5,59 \cos 153,4^\circ = \\
        &= 1 -5,59 \cdot 0,89 \simeq -3.98\\ \\
y_{O_P} &= y_P +r \sen\beta = \\
        &= y_P +5,59 \sen 153,4^\circ = \\
        &= 1 +5,59 \cdot 0,45 \simeq 3.52
\end{align*}
}{
\hspace{-5mm}\scalebox{1}{\osculatoreesea}
}
...
\subsection{Curvatura di una linea}
I grafici delle pagine precedenti ci hanno mostrato alcune rette e tante 
curve. 
Calcolando le derivate abbiamo iniziato ad analizzare la forma delle 
curve-grafici delle funzioni regolari. Anche se più avanti renderemo più 
generali queste nozioni, al momento sappiamo che:
...
\item con la derivata seconda mostriamo come variano le pendenze, perciò 
sappiamo dire se in un certo intervallo la curva mostra una concavità 
verso l'alto (le pendenze crescono quando \(x\) aumenta) 
oppure una concavità verso il basso (le pendenze diminuiscono quando \(x\) 
aumenta).
\end{itemize}
La maggior parte dei grafici ha una forma curvilinea, per questo ora 
concentriamo lo studio sulla curvatura di un grafico. 
...
curva: geometricamente è la pendenza, che corrisponde al coefficiente 
angolare della tangente alla curva nel punto considerato. 
Conoscendo l'angolo \(\alpha\), la pendenza 
\(f'(x)\) in quel punto è \(f'(x)=\tg(\alpha)\) e quindi 
\(\alpha=\arctan(f'(x))\).\\
Dal teorema dell'incremento si ha:
...
\]
%TODO   disegno tipo p.396, magari in una semicirconf come un grafico già 
% pronto
\begin{center}  disegno\end{center}


Dove il grafico mette in evidenza una forte curvatura, lì abbiamo un 
forte cambio di direzione in un tratto breve. Al contrario dove la curvatura 
è minore, il cambio di direzione è minore oppure avviene lungo un percorso 
più lungo. C'è quindi una proporzionalità inversa fra la curvatura \(k\) e la 
lunghezza \(ds\), a parità di angolo \(d\alpha\), e c'è una proporzionalità 
diretta fra \(k \) e \(d\alpha\), a parità di \(ds\): possiamo scrivere
...
una misura della curvatura di un grafico in ogni suo punto. 
\begin{esempio}
Analizziamo la curvatura del grafico della funzione \(f(x)=\dfrac{1}{x}\), 
che abbiamo già visto molte volte. Limitiamoci all'intervallo \(x>0\). 
Il grafico non ha quasi curvatura agli estremi dell'intervallo mentre ne ha 
una molto accentuata per \(y=x\). 
Vediamo l'espressione di \(k\), che essendo diversa da punto a punto, sarà 
\(k=k(x)\).
\begin{align*}
 f(x) &= \dfrac{1}{x}\quad \text{ con } x>0 
 \qquad f'(x)=\dfrac{-1}{x^2}\qquad f''(x)=\dfrac{2}{x^3}\\
k(x)&=\dfrac{f''(x)}{\quadra{1+(f'(x))^2}^{\frac{3}{2}}}=
\dfrac{\dfrac{2}{x^3}}{\quadra{1+\tonda{\dfrac{-1}{x^2}}^2}^{\frac{3}{2}}}=
\dfrac{\dfrac{2}{x^3}}{\quadra{1+\dfrac{1}{x^4}}^{\frac{3}{2}}}=
\dfrac{\dfrac{2}{x^3}}{\dfrac{[x^4+1]^\frac{3}{2}}{x^6}}=
...
C'è un secondo modo, molto espressivo, di indicare la curvatura. 
Come si vede dall'ultimo esempio, se non consideriamo il segno, il risultato 
è il reciproco del raggio della circonferenza. 
Infatti abbiamo definito la curvatura \(k\) come \(k=\dfrac{d\alpha}{ds}\), 
il rapporto fra il cambio di direzione e la lunghezza del tratto percorso.

Nelle circonferenze, se \(\alpha\) si misura in radianti, vale la relazione 
\(ds=rd\alpha\), per cui il raggio è \(r=\dfrac{ds}{d\alpha}=\dfrac{1}{k}\). 
Da qui viene l'idea di esprimere la curvatura di un grafico in un suo punto 
attraverso il raggio del cerchio che in quel punto approssima la curva al 
meglio. Tale cerchio è chiamato \emph{cerchio osculatore}.
...
\[
 r=\dfrac{\quadra{1+(f'(x))^2}^{\frac{3}{2}}}{f''(x)}
\]
e il segno che risulterà alla fine del calcolo ci indica se la circonferenza 
si trova sopra o sotto la curva, come puoi verificare negli ultimi esempi.

Non è difficile trovare il centro \(O\) del cerchio osculatore: 
se \(P\) è il punto della curva che stiamo esaminando, il raggio \(r=OP\) è 
un segmento che appartiene alla normale alla curva, passante per \(P\). 
Il calcolo si svolge nel prossimo esempio.

Quanto alla curvatura di una retta, che risulta \(0\) come abbiamo visto, 
il raggio del cerchio osculatore risulterà infinito, come è giusto se si 
immagina una circonferenza che ha il suo centro a distanza infinita dalla 
retta.
...
\(r=\dfrac{\quadra{1+(f'(x))^2}^{\frac{3}{2}}}{f''(x)}=
 \dfrac{\quadra{1+2^2}^{\frac{3}{2}}}{2}\simeq 5,59\).\\
 Dato che \(\alpha\), l'inclinazione della tangente, è 
\(\alpha=\arctan(f'(1))=\arctan(2)=63,4^\circ\), l'inclinazione della 
normale passante per \(P\) è \(\beta=\alpha+90^\circ=153,4^\circ\).\\
Le coordinate del centro \(O_P\) del cerchio osculatore in \(P\) saranno 
quindi:
\begin{align*}
x_{O_P} &= x_P+\cos\beta=x_P+\cos 153,4^\circ=1-0,89=0,11\\
y_{O_P} &=y_P+\sen\beta=y_P+\sen 153,4\circ=1+0,45=1,45.
\end{align*}
Da notare, infine, che le due circonferenze si trovano sopra la curva, dato 
che la curvatura \(k\) è positiva perché \(f''(x)\geqslant 0\).
\end{esempio}
...
% \noindent Ipotesi: \(f(x)=\sqrt{x}\) \tab 
% Tesi: \(df(x)\sim\dfrac{dx}{2\sqrt{x}}\).
...
\end{teorema}
% \noindent Ipotesi: \(f(x)=f_1(x)\pm f_2(x)\) \tab 
% Tesi: \(df(x)=df_1(x)\pm df_2(x)\).
...
% Tesi: 
% \(d\mathit{A}(x)=db(x)\cdot h(x)+b(x)\cdot dh(x)+ db(x)\cdot dh(x)\).
