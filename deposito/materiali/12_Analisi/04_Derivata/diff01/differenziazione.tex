...
\(-2\) & \(\dfrac{31}{3}\)     & \(\dfrac{-10}{3}\) \\
\(-1\) & \(\dfrac{22}{3}\)     &  \(\dfrac{-8}{3}\) \\
\(0\)  & \(5\)                 & \(-2\)  \\
\(1\)  & \(\dfrac{10}{3}\)     & \(\dfrac{-4}{3}\) \\
\(2\)  & \(\dfrac{7}{3}\)      & \(\dfrac{-2}{3}\)\\
\(3\)  & \(2\)                 & \(0\)\\
\(4\)  & \(\dfrac{7}{3}\)       &\dots \\
\(5\)  & \(\dfrac{10}{3}\) \\
\(6\)  & \dots   \\
...
\end{teorema}
\noindent Ipotesi: \(f=f_1 + f_2\) e esiste \(f'(x)\).\tab Tesi: \(f'=f'_1 + 
f'_2\).\\
(Ovvero: La derivata di una somma è la somma delle derivate)

\begin{proof}
Consideriamo il Rapporto Differenziale, calcolato in \(x\), dove \(f_1\), 
\(f_2\) e quindi anche \(f\), hanno valore. Allora, per ogni infinitesimo non 
nullo \(dx\):
\begin{align*}
 RD&=\dfrac{df(x)}{dx}=\dfrac{d[f_1(x)+ f_2(x)]}{dx}=\dfrac{[f_1(x+dx)+
     f_2(x+dx)]-[f_1(x) + f_2(x)]}{dx}=\\
 &= \dfrac{[f_1(x+dx)-f_1(x)] + [f_2(x+dx)-f_2(x)]}{dx}= 
 \dfrac{f_1(x+dx)-f_1(x)}{dx}+\dfrac{f_2(x+dx)-f_2(x)}{dx}=\\
  &=\dfrac{df_1(x)}{dx}+\dfrac{df_2(x)}{dx}.
\end{align*}
...
\(dx\). Di conseguenza anche i Rapporti Differenziali per \(f_1(x)\) e per
\(f_2(x)\) sono numeri finiti e le loro parti standard esistono e non possono 
cambiare valore cambiando \(dx\).\\
Per le proprietà della funzione parte standard vale quindi:
\[
 f'(x)=\pst{\dfrac{df}{dx}}=\pst{\dfrac{df_1(x)}{dx}+\dfrac{df_2(x)}{dx}}=
 \pst{\dfrac{df_1(x)}{dx}}+\pst{\dfrac{df_2(x)}{dx}}=f'_1(x)+f'_2(x).
 \]
\end{proof}

\begin{osservazione}
 Il teorema vale anche per la differenza, in modo analogo e si può applicare 
anche alla somma algebrica di più funzioni, come avviene comunemente nei 
polinomi.
\end{osservazione}
