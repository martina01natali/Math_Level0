...
cartesiano sono rappresentate da curve. Allora le cose cambiano.\\
...
\item Il Rapporto Incrementale (R.I) fra le coppie di valori di una funzione è 
...
\begin{esempio}
Fissiamo su curva una due punti: uno fisso: $A$ e l'altro mobile, cioè in grado di 
spostarsi lungo la curva dalla posizione più lontana $H$, alla più prossima ad $A$,
cioè, oltre $B$, fin quasi a sovrapporsi con $A$.

\begin{figure}[h]
\begin{inaccessibleblock}[Verso la tangemte a una curva.]
 \begin{center}
\secanticurva
 \end{center}
\end{inaccessibleblock}
...
Tracciamo le secanti che uniscono $A$ con le varie posizioni di $P$. 
...
$\forall n$, siamo nel campo degli infinitesimi: cambia la natura del Rapporto 
Incrementale che avevamo imparato a calcolare. Il R.I. si trasforma: da un
rapporto fra quantità finite a un rapporto fra infinitesimi, quindi non
possiamo essere certi che fornisca un risultato e di quale natura sia.\\
Ma sappiamo che, se il rapporto è calcolabile e dà luogo a un numero finito,
fornirà un numero indistinguibile dalla pendenza della tangente.

\begin{definizione}
 Si dice Rapporto Differenziale della funzione $f(x)$, relativo a $x_0$
 il rapporto $\frac{df(x)}{dx}\big|_{x=x_0}$ fra il differenziale della funzione 
 e quello della variabile, calcolati nel punto $x_0$. \\
 $\frac{df(x)}{dx}\big|_{x=x_0}=\frac{f(x_0+dx)-f(x_0)}{dx}$.
\end{definizione}

DISEGNO CON MICROSCOPIO DELLA TANGENTE ALLA STESSA CURVA
