...
retta che disega il suo grafico.
  %: \(\mathit{D}\quadra{k}=0\).
\end{teorema}
...
disegno che spesso si usa per rappresentare il prodotto fra due binomi, senza 
soffermarci sui suoi limiti: infatti si potrebbe discutere sul fatto che un'area 
rappresenti un prodotto tra funzioni.
...
funzione:
 \[d(x^n) \sim nx^{n-1}dx\]
e la sfruttiamo per ricavare la formula della derivata.
\[
 \forall dx\ne 0:\quad
...
\]
\end{proof}
Ora disponiamo di un'arma molto più potente, che, insieme alle precedenti, ci 
permette di calcolare derivate come questa.
\begin {esempio}
 Derivare la funzione polinomiale \(f(x)= -\dfrac{4}{15}x^5+ 
\dfrac{2}{3}x^3-13kx\).
 \begin{align*}
...
\end {esempio}
 ...
 \subsubsection{Funzioni potenza a esponente intero}
 \label{subsubsec:derivata_potenze}
