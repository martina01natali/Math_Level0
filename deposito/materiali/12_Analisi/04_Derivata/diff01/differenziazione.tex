...
\): non potremmo derivarli separatamente e poi unire i risultati attraverso una 
somma? È quello che stiamo per mostrare con i prossimi teoremi.
...
conclusione.
\end{proof}
Abbiamo appena usato il termine ``derivabile'', che ha un significato 
intuitivo. Comunque definiamolo.
\begin{definizione}
...
sia calcolabile per ogni \(x\) del suo dominio.
\end{definizione}
...
\]
Poiché il risultato non cambia \(\forall dex \ne 0\), la tesi è dimostrata.
\end{proof}
...
\(f(x)=\dfrac{1}{x^2}\), e così via, con \(x\ne 0\). Ma possiamo anche 
arrivarci in un altro modo, basta riscrivere le due funzioni precedenti come 
potenze.
...
f'(x)&= zx^{z-1}=-nx^{-n-1}.
\end{align*}
Resta perciò dimostrato il seguente teorema.
\begin{teorema}
 La regola della derivata di una funzione potenza vale anche con esponente se 
l'esponente è un intero negativo.
\end{teorema}
...
\begin{enumerate} [noitemsep]
 ...
 derivata della funzione potenza (\(\alpha \in \Q\))
\end{enumerate}
 

E abbiamo giustificato le regole di derivazione per alcune funzioni che sono 
...
\begin{enumerate} [noitemsep]
 ...
 \item \(\dfrac{d\tonda{\dfrac{f}{g}}}{dx}=\frac{f'\cdot g-f\cdot 
g'}{g^2}\)\tab 
