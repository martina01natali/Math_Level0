...
% La semplicità dei calcoli precedenti lascia intuire la ragione del 
% successo del calcolo con gli infinitesimi. 
% Questo tipo di calcolo fiorì per 150 anni a partire dall'epoca di 
% Newton e Leibniz. 
...
% infinitesimi al termine dei calcoli. 
% 
% Oggi i matematici conoscono meglio la materia e queste difficoltà 
% sono superate. 
...
% gravi, un punto che si dà sempre per scontato nella fisica classica, 
% ma non lo è per i matematici e per i logici.
% 
% Il ragionamento dell'esempio precedente si basa sull'assunto che il 
% tempo scorra in modo uniforme. 
% Se il tempo scorresse a scatti, anche minuscoli, quei calcoli non 
% sarebbero possibili. Si dice infatti che il tempo \(t\) è una 
% variabile continua, cioè assume tutti i valori, dal minimo al massimo, 
% con regolarità, senza salti.
% 
% \begin{definizione}
...
% posizione $x$ sull'asse dei numeri reali. Infatti sappiamo che la retta 
% reale non ha buchi. 
% A maggior ragione, è una variabile continua anche $x\in\IR$, 
% con la quale rappresentiamo la posizione sull'asse iperreale. \\
% Viceversa una variabile che pesca i suoi valori in un insieme formato 
% da numeri isolati, cioè con differenze finite fra l'uno e l'altro, 
% si dice \emph{variabile discreta}. 
% \begin{definizione}
...
% assumendone tutti i valori, dal minimo al massimo. In matematica si 
% scrive così:
% \(t\in \quadra{0;\ 3,6}\). Le parentesi quadre sono importanti, indicano 
% che gli estremi dell'intervallo sono valori possibili, sono inclusi.
...
% aperto & $\intervaa{a}{b}$ & estremi esclusi
% \end{tabular}
% \label{tab:diff_tipiinterv}
% \end{center}
% Tutti i tipi di intervallo precedenti, nella retta reale o iperreale, 
% sono continui, a meno di indicazioni diverse. Se un intervallo 
% \(\intervcc{a}{b}\) 
% contiene un punto (o più punti) di discontinuità, per esempio \(d\), 
% allora occorre usare indicazioni diverse: 
% \(\intervca{a}{d}\cup\intervac{d}{b}\)
% 
% La differenza \(b-a\) fra due numeri della retta iperreale può essere 
% positiva, negativa o nulla. 
% Indicheremo con \(\Delta\) la differenza fra due numeri standard, 
% cioè una differenza finita, mentre, se la differenza è infinitesima, 
% sarà indicata con \(\delta\) oppure \(\epsilon\) o altra lettera 
% minuscola dell'alfabeto greco.
...
\section{Pendenza e derivata}
\label{sec:differenziazione_pendenzaederivata}

Il problema precedente equivale a calcolare la pendenza del grafico di 
una funzione in un punto. Infatti nel piano \emph{tempo - spostamento}
la velocità è rappresentata dalla pendenza della curva che rappresenta 
il movimento.

\subsection{Pendenza di una funzione in un punto}
\label{subsec:differenziazione_pendenzafunzione}

Una retta ha in ogni suo punto la stessa pendenza e possiamo definire come 
pendenza il rapporto tra lo spostamento in verticale e lo spostamento in 
orizzontale muovendoci da un punto \(A\) qualsiasi a un altro punto \(B\):
\[pendenza = \dfrac{y_B - y_A}{x_B - x_A} = \dfrac{\Delta y}{\Delta x}\]
Qualunque sia la coppia di punti distinti scelta, otterremo sempre lo 
stesso valore che corrisponde al coefficiente angolare della retta.

Ma quanto detto per le rette, non vale per tutte le funzioni, 
in generale, la pendenza di una funzione varia da punto a punto. 
Possiamo avvicinarci alla pendenza in un punto \(P\) scegliendo come 
altro estremo dello spostamento, un punto molto vicino, ma averemo sempre 
una pendenza media.
E chi ci assicura che questa pendenza media sia vicina alla pendenza in
quel punto?
Se nel punto \(P\) la curva ha un salto, il metodo precedente non ci 
fornisce dei valori sensati.

Se la curva è sufficientemente regolare, possiamo pensare che, se 
l'intervallo \(\Delta x\) è piccolo, la pendenza media relativa a questo 
intervallo sia vicina alla pendenza nel punto \(P\).
% \begin{center} \parabolaequasitangente \end{center}
Se pongo \(P'\) a una \emph{distanza trascurabile} 
da \(P\) la pendenza che calcolo dovrebbe avere una 
\emph{differenza trascurabile}
dalla pendenza della curva in quel punto.

Gli iperreali ci mettono a disposizione dei numeri trascurabili rispetto
a tutti i numeri reali: sono gli infinitesimi.
Possiamo perciò usarli per ottenere la pendenza media in un intervallo
infinitesimo:
\[\fstar[pendenza] = \dfrac{f(x + \epsilon) - f(x)}
                           {\tonda{x + \epsilon} - x}\]
Il valore \(\fstar[pendenza]\) che si ottiene è un numero iperreale, 
ma la pendenza che cerchiamo deve essere un \emph{numero reale} quindi 
passiamo dagli iperreali ai reali usando la funzione 
\emph{parte standard}.
...
\begin{esempio}
Applichiamo quanto visto a una curva abbastanza tranquilla. Calcola la 
pendenza della parabola:
\(f(x) = -\dfrac{1}{4}x^2+2x +1\) \quad
nel suo punto di ascissa:~\(x_P=8\).
...
\affiancati{.58}{.40}{
Ora possiamo calcolare la pendenza della retta per questi due punti:
\begin{align*}
% m &= \frac{f(x + \delta) - f(x)}{x + \delta - x}=
\fstar[pendenza] &= \frac{y_{P'} - y_{P}}{x_{P'} - x_{P}}=
...
Si intuisce che la parola incremento significa aumento, cioè differenza fra 
due valori nel passare dall'uno all'altro. In matematica si chiama 
incremento anche quando il secondo valore è minore del primo, sicché la 
...
\begin{align*}
 \Delta f &= f(7) - f(3) = 
             \tonda{\dfrac{7^2}{4}-7-3} - \tonda{\dfrac{3^2}{4}-3-3} =\\
...
quantità \(a\) nel passare da \(x_0\) a \(x_0+a\). \(a\) può essere positivo o 
negativo.
...
Una funzione \(f(x)\) è costante se qualunque sia il valore di $x$ il risultato 
...
alla più prossima ad $A$, cioè oltre $P_7$, fin quasi a sovrapporsi con $A$.
...
per $x\ge 0$ e $m=-1$ per $x<0$. Quale è la pendenza giusta della tangente per
...
\begin{center} \scalebox{.8}{   \esp} \end{center}
 \end{minipage} 
  \hfill
 \begin{minipage}[]{.42\textwidth}
\begin{center} \scalebox{.8}{   \tangentiesp} \end{center}
 \end{minipage}
\end{inaccessibleblock}
\label{}
