...
\begin{teorema}
 Se una funzione è la somma di due funzioni, anche la sua 
 derivata, se c'è, sarà la somma delle due derivate.
\end{teorema}
\noindent Ipotesi: \(f=g + h\) e esiste \(f'(x)\).\tab Tesi: \(f'=g' + 
h'\).\\
...
\(\mathit{S'}(x),\ b'(x),\ a'(x)\).\\ 
Tesi: \(\mathit{S'}(x)=b'(x)\cdot a(x)+b(x)\cdot a'(x)\).
...
\label{subsubsec:derivata_f_per_k}
 \begin{teorema}
La derivata di una funzione \(f\) data dal prodotto \(k\cdot g(x)\) fra una 
funzione derivabile \(g(x)\) e una costante \(k\) è il prodotto fra la costante 
\(k\) e la derivata \(g'(x)\). \end{teorema}
...
\end{align*}
Ora che il meccanismo è chiaro, abbiamo la regola per il differenziale di questa 
...
con \(b(x)\neq 0\) \tab 
Tesi: 
\(da(x) \sim \dfrac{d\mathit{A}(x) \cdot b(x)-\mathit{A}(x) \cdot db(x)}
             {\tonda{b(x)}^2}\)
...
\dfrac{d\mathit{S}(x) \cdot b(x) - \mathit{S(x)} \cdot 
 db(x)}{dx}\cdot\dfrac{1}{\tonda{b(x)}^2}=\\
    &=\tonda{\dfrac{d\mathit{S}(x) \cdot b(x)}{dx}-\frac{\mathit{S(x)} \cdot 
 db(x)}{dx} }\cdot\dfrac{1}{\tonda{b(x)}^2}= 
\dfrac{\dfrac{\mathit{S(x)}}{dx}b(x)-\mathit{S(x)}\dfrac{db(x)}{dx}}{\tonda{b(x) 
}^2}.
\end{align*}
...
usati, e si ottiene così \(
 a'(x)=\dfrac{\mathit{S'(x)}\cdot b(x)-\mathit{S(x)}\cdot b'(x)}{(b(x))^2}\).
...
\begin{enumerate} [noitemsep]
 \item \(\dfrac{d(k)}{dx} = 0\) \tab  derivata di una costante;
 \item \(\dfrac{d(x)}{dx} = 1\) \tab derivata della funzione identica;
 \item \(\dfrac{d(x^\alpha)} {dx} = \alpha x^{\alpha-1}\) \tab 
 derivata della funzione potenza (\(\alpha \in \R\)).
\end{enumerate}
\begin{osservazione}
\label{oss:regola3}
...
\begin{enumerate} [noitemsep]
 \item \(\dfrac{d(k\cdot f)}{dx}=kf'\) \tab 
 derivata del prodotto per una costante;
 \item \(\dfrac{d\tonda{f\pm g}}{dx}=f'\pm g'\) \tab 
 derivata di una somma o differenza;
 \item \(\dfrac{d(f\cdot g)}{dx}= f'\cdot g+f\cdot g'\) \tab 
 derivata del prodotto;
 \item \(\dfrac{d\tonda{\dfrac{f}{g}}}{dx}=\dfrac{f'\cdot 
g-f\cdot g'}{g^2}\)\tab 
...
\begin{align*}
 ...
 \dfrac{1}{2x^{\frac{1}{2}}}=\dfrac{1}{2\sqrt{x}},\quad \text{ con  } x\ne 0.
\end{align*}
\end{osservazione}
...
infinitesimo \(dx \).\\
Poi, poiché esiste \(f'(g)\), dal teorema dell'incremento si deduce che per ogni 
...
Il caso generale, in cui \(y=f(x)=\sqrt[m]{x^n}=x^\frac{m}{n}\) si può 
dimostrare con lo stesso procedimento, oppure si può utilizzare  
il Teorema \ref{teo:diff01_dericomp}, 
unito alla dimostrazione che si legge  nelle righe precedenti. 
\end{proof}
...
questa sezione ci occupiamo delle funzioni trascendenti e dovremo prima 
imparare a differenziarle.
...
\end{inaccessibleblock}
\label{}
\begin{center} Il grafico di \(y=a^x\), con alcune sue tangenti, e l'inviluppo 
delle tangenti.\end{center}

Anche se i due grafici non sono identici, le pendenze delle tangenti pare che 
abbiano un andamento anch'esso esponenziale: la derivata della 
...
\(a^x\) e il Rapporto Differenziale di \(f(x)\) calcolato per \(x=0\).
...
\begin{esempio}
  ...
  \(f'(x)=\tonda{2^{g(x)}\ln 2} g'(x)=   2^{x^2}\ln 2\cdot 2x=xe^{x^2+1}\ln 2\).
\end{esempio}
...
derivabili: poiché \(\ln x\) esiste per \(x>0\), i ragionamenti valgono solo per 
\(x>0\)
\end{proof} 
 \end{minipage}
\end{inaccessibleblock}
