...
FIN QUI ABBIAMO PARLATO DI FUNZIONI E FUNZIONI INVERSE, ORA DOBBIAMO PASSARE 
ALLA DERIVATA DI UNA FUNZIONE INVERSA.
...
Possiamo osservare che \(f'(x)\)  è il reciproco di \(g'(x)\) dato che 
il loro prodotto è \(1\). 
In questo calcolo, inoltre, bisogna segnalare che \(g'(x)\) esiste se 
\(x\ne0\).
\end{esempio}
...
dell'altra,
    ...
    \item 
\(\Deriv{f^{-1}(x)}=\dfrac{1}{\Deriv{(f^{-1}(x)}}\);
    \item \(\Deriv{\ln x}=\dfrac{1}{\Deriv{e^{g(x)}}}
...
\begin{center} \scalebox{.8}{\esplog} \end{center}
 ...
 \begin{teorema}
  ...
  logaritmo naturale è: \(\Deriv{\ln x}=
...
\label{teo:funzione_potenza_generica}
  ...
  \(\Deriv{x^\alpha}=(\alpha-1)x^\alpha\), \(\forall\alpha\).
\end{teorema}
...
\begin{teorema}
  La derivata della funzione \(f(x)=\cos x\) è \(\Deriv{\cos x}=
  -\sen x\).
\end{teorema}
...
\begin{teorema}
   La derivata della funzione \(f(x)=\tg x\) è \(\Deriv{\tg x}=
...
\dfrac{\Deriv{\sen x}\cdot \cos x-\sen x\cdot 
  \Deriv{\cos x}} {\cos^2 x}=
