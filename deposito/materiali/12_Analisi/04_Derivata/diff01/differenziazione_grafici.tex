...
\newcommand{\funzioneederivata}{% 
  ...
  \foreach \xp/\yp in {-2/10.33, -1/7.33, 0/5, 1/3.33, 2/2.33,
                       3/2, 4/2.33, 5/3.33, 6/5, 7/7.33, 8/10.33} 
    \filldraw (\xp, \yp) [blue!50!black] circle(2pt);
  \foreach \xp/\yp in {-3/-4, -2/-3.33, -1/-2.67, 0/-2, 1/-1.33, 2/-0.66,
                       3/0, 4/0.67, 5/1.33, 6/2, 7/2.67, 8/3.33} 
    \filldraw (\xp, \yp) [green!50!black] circle(2pt);
    }
}
...
\newcommand{\differenzialeprodotto}{% rettangolo con gnomone i
  ...
  \disegno[4]{
    ...
    \draw (\xa, \ya) node [below] {\(0\)} node [left] {\(0\)} -- 
    (\xb, \ya) node [below] {\(b \approx b + \Delta b\)} -- 
    (\xb, \yb) -- 
    (\xa, \yb) node [left] {\(a \approx a + \Delta a\)} -- cycle;
    ...
    \end{scope}
    \begin{scope}[font=\fontsize{6}{6}] %\scriptsize % \small
	    \draw (1.35*\xb, 1.7*\yb) node{\(b \cdot \Delta a\)};
	    \draw (1.57*\xb, 1.37*\yb) node [rotate=90] {\(\Delta b \cdot a\)};
% 	    \draw (1.63*\xb, 1.85*\yb) node {\(\Delta b \cdot \Delta h\)};
