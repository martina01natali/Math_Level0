...
\[\int_{}^x f~dx = \int_{}^a f~dx + \int_{a}^x f~dx \sRarrow
  \mathcal{S}(x) = \mathcal{S}(a) + \mathcal{S}(a,~x)\]

Ora, non sappiamo quanto sia \(\mathcal{S}(a)\) cioè la superficie sottesa 
alla funzione fino ad \(a\), ma di sicuro non dipende ad \(x\), cioè, 
...
quello atteso, solo per una costante \(\mathcal{C}\).
Quindi:
\[\int f~dx = \int_{a}^x f~dx + \mathcal{C}\]
...
\begin{minipage}{.39 \textwidth}
\begin{inaccessibleblock}[] 
  \funzintx{1}{2}{4}{7}{y=x}
%   \funzintx %{1}{0}{4.7}{y=mx+q}
\end{inaccessibleblock}
\end{minipage}
...
sospetto, se ci ricordiamo qualcosa sulle derivate. 
...
funzione \(f\). Questo rettangolino infinitesimo sarà:
\[d S(x)\]
L'area di questo rettangolino vale:
\[f(x)~dx\]
Quindi:
