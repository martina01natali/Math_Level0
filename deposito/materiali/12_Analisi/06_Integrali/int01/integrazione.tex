...
\[se F(x) = \int f(x)~dx allora \int_a^b f(x)~dx = F(b) - F(a)\]

\begin{esempio}
Calcola l'area sottesa alla funzione \[f: x \mapsto 2 x^2 -x +3\] tra~1 e~3.

\begin{minipage}{.59 \textwidth}
Calcoliamo un integrale indefinito della funzione \(f\):
\[\int \tonda{2 x^2 -x +3}~dx = 
  2 \int x^2~dx -\int x~dx + \int 3~dx =
  \frac{2}{3}x^3 - \frac{1}{2}x^2 +\]
\end{minipage}
\hfill
\begin{minipage}{.39 \textwidth}
\begin{inaccessibleblock}[]  
