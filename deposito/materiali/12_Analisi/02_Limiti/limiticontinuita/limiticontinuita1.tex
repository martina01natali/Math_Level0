...
Europa. Il disegno è tratto dall'originale.}~

\affiancati{.64}{.34}{
\agnesia}{
Recuperiamo il brillante e suggestivo ragionamento di M.G.Agnesi a proposito 
delle relazioni che esistono fra un angolo \(\Delta\), e 
quindi l'arco \(CD\), se infinitesimi, e le relative funzioni goniometriche.}
\label{par:agnesi}

\begin{enumerate}[noitemsep]
 \item (Sul coseno) Dato che
il diametro \(GC\) è una quantità finita ed è la base del triangolo rettangolo 
\(GCD\), per il 2° Teorema di Euclide: \(GB:BD=BD:BC\).\\
\(\dfrac{GB}{BD}\) è il rapporto fra una quantità finita ed una 
infinitesima ed è quindi un infinito. Quindi deve essere altrettanto infinito 
il secondo rapporto di questa proporzione. Allora il segmento \(BC= 
AC-AB=1-\cos~\Delta\) è infinitesimo rispetto a \(BD=\sen~\Delta\) ed è per 
questo un infinitesimo del secondo ordine.\\
...
all'altezza, lo stesso succederà nel secondo triangolo. Ma la base di questo, 
...
indistinguibili;
\item il limite \(\lim_{x \rightarrow 0} \dfrac{1-\cos x}{x}\) 
deve risultare nullo e il \(\lim_{x \rightarrow 0} \dfrac{1-\cos x}{x^2}\) 
deve avere un valore finito.
\end{itemize}

Su tutto questo ritorniamo con calcoli più precisi.
\paragraph{Funzione seno}~
\label{limiti:par_f_seno}

Osservando il grafico della funzione seno a pag.~\pageref{gra:goniometriche}, 
si intuisce che:
...
Per capire il comportamento della funzione \(sen~x\) quando l'arco (e 
l'angolo \(\delta\)) si avvicina a zero, seguiamo il geniale ragionamento di 
Archimede, a proposito del rapporto fra circonferenza e diametro, in aggiunta a 
quanto abbbiamo visto nella pagina precedente.\\
Archimede immaginò la circonferenza racchiusa fra un poligono regolare inscritto 
e uno circoscritto e determinò la lunghezza del lato in base al diametro. 
Raddoppiando progressivamente i lati (fino a 96 lati!) e aggiornando i calcoli, 
si accorse che il rapporto fra i perimetri e i diametri si assestava 
progressivamente intorno al valore \(\frac{223}{71}\simeq 3,141\). Dato che la 
circonferenza è intermedia fra il poligono inscritto e quello circoscritto, 
questo numero, che noi chiamiamo \(\pi\), ci indica il rapporto fra una 
circonferenza e il suo diametro. Questo vale per tutti i cerchi, di qualsiasi 
raggio.\\
...
al seno dell'angolo al centro infinitesimo \(sen~\delta\):
...
Se l'angolo \(\delta\) è infinitesimo, i due raggi che lo formano nel disegno a 
grandezza naturale risultano sovrapposti: \(AC\) coincide con \(AD\), 
\(\delta\) sembra zero, il coseno sembra lungo quanto il raggio e il seno non si 
riesce a distinguere, come l'arco. Ci vuole un microscopio infinito (M1) per 
individuare l'arco \(CD\), che appare come un segmento verticale, raggiunto 
agli estremi dai due raggi orizzontali e paralleli. 
Con un nuovo ingrandimento infinito (M2) sul 
punto \(C\) cogliamo la distinzione fra il coseno e il raggio, cioè 
\(BC=1-\cos\delta\), che, come ha ben spiegato l'Agnesi, è un segmento 
...
\end{minipage}
Puntiamo allora M4 su \(E\) per cogliere la differenza \(CF-CE\) fra il seno e 
la tangente, che ora possiamo vedere perché l'ingrandimento complessivo è del 
terzo ordine.

In questo modo si mette in evidenza che se \(\delta\) è infinitesimo,  
\(\delta\), \(\sen~\delta\) e \(\tan\delta\) sono infinitesimi indistinguibili 
(M1 e M5),  mentre \(1-\cos\delta\) è un infinitesimo del secondo ordine 
rispetto a \(\delta\) (M2).

Tutto questo giustifica i limiti notevoli che seguono, alcuni dei quali si 
dimostrano anche con il calcolo diretto.
\[\lim_{x \rightarrow 0}\frac{\sen x}{x} = 
  \pst{\frac{\sen \delta}{\delta}} = 1\]
% Si conclude che \(\sen \delta \text{ è diverso da } \delta\) solo per 
...
Possiamo dimostrare i seguenti due limiti che abbiamo giustificato 
con i disegni:
...
% quantità finita ed è la base del triangolo rettangolo \(GCD\). \\
% Per il 2° Teorema di Euclide:
% \begin{center} \(GB:BD=BD:BC\). \end{center}}
% 
% \(\dfrac{GB}{BD}\) è il rapporto fra una quantità finita ed una 
% infinitesima 
% ed è quindi un infinito. Quindi deve essere altrettanto infinito il secondo 
% rapporto di questa proporzione. Allora il segmento \(BC= AC-AB=1-\cos~\delta\) 
% è infinitesimo rispetto a \(BD=\sen~\delta\) ed è per questo un infinitesimo 
% del secondo ordine. L'ordine di infinitesimo si può evidenziare rielaborando la 
% proporzione: \(BC=\dfrac{BD^2}{BG}\). Il denominatore è una quantità 
% finita, per cui la frazione esprime una quantità infinitesima al 
% quadrato.
% 
% Tutto questo giustifica il valore finito del limite
