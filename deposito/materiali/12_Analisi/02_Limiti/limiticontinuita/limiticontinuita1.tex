...
\subsubsection{Se il limite finito non esiste}
...
per \(x\) che tende a un valore \(c\) se \(f(x)\) è infinitamente vicino a 
\(L\) per tutti gli \(x\) infinitamente vicini a \(c\) e minori di \(c\).
...
In modo simmetrico si definisce il limite destro.
...
Studiamo il limite per \(x\) che tende a \(4\)
...
valore di \(\epsilon\) ma solo se solo se \(\epsilon > 0\).
Quindi non possiamo calcolare \emph{il limite} ma solo 
\emph{il limite destro}. Quindi sospendiamo il calcolo del limite, 
...
\subsection{Se non esiste \(\pst{f(c+\epsilon)}\)}
\label{subsec:cont_limiti_nonsempreesiste}
...
\begin{comment}
% Da usare più avanti... forse qui.

Una funzione importante per esplorare situazioni strane attorno allo zero è:
\[f(x) = \sen \frac{1}{x}\]
Più diventa grande \(\abs{x}\) più \(\frac{1}{x}\) si avvicina a \(0\) e 
quindi anche \(\sen \frac{1}{x}\) si avvicina all'asse \(x\).
\begin{center} \limitesinunosux \end{center}
Più diventa grande \(\abs{x}\) più \(\frac{1}{x}\) si avvicina a \(0\) e 
quindi anche \(\sen \frac{1}{x}\) si avvicina all'asse \(x\).

Ma la parte interessante della funzione è quella vicino allo \(0\):
man mano che \(x\) si avvicina a \(0\), \(\frac{1}{x}\) diventa sempre più 
grande e quindi \(\sen \frac{1}{x}\) oscilla in modo sempre più fitto 
tra \(-1 \stext{e} +1\).

Anche quando ci avviciniamo infinitamente a \(0\) il comportamento continua 
allo stesso modo, per transfer.
