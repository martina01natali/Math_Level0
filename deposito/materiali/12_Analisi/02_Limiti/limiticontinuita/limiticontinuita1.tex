...
\subsubsection{\mathversion{bold}$\sen x$}
Conoscendo la funzione seno, risulta abbastanza evidente che:
\[\lim_{x \rightarrow }\tonda{\sen x} = 0\]
e che 
\[\lim_{x \rightarrow \infty}\tonda{\sen x} = \text{ non esiste}\]
...
grafica:
\[\lim_{x \rightarrow }\frac{\sen x}{x} = 1\]
...
\subsubsection{Numero di Eulero}
...
% Quando dobbiamo calcolare il limite di certe funzioni esponenziali è 
% importante ricordare la definizione del numero di Eulero (o di Nepero):
% \[e=\tonda{1+\frac{1}{M}}^M = \tonda{1+\epsilon}^{\frac{1}{\epsilon}}\]
...
Dove le uguaglianze hanno i seguenti motivi:
\begin{enumerate} [nosep]
 \item uno sporco trucco: se al posto di \(\dfrac{k}{N}\) 
scrivo \(\dfrac{1}{M}\) allora al posto di \(N\) dovrò scrivere \(N=kM\);
...
Prima di affrontare il prossimo esempio ricordiamo che se 
\(a^\epsilon = 1+\delta\) allora \(\epsilon\) è l'esponente da dare a \(a\) 
per ottenere \(1+\delta\): \(\epsilon = \log_{a}{\tonda{1+\delta}}\)
