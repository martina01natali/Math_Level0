...
\(f(x)\) è infinitamente vicino a \(f(4)\), cioè se \(\forall \epsilon\)
\(f(4 + \epsilon) - f(4) \approx 0\)
...
questa affermazione equivale a dire che per ogni infinitesimo \(\epsilon\) 
si ha che \(f(c+\epsilon) \approx f(c)\), 
mentre per gli estremi dell’intervallo ci si accontenterà di dire che per 
ogni infinitesimo \emph{positivo} \(\epsilon\) si ha che 
\(f(a+\epsilon) \approx f(a)\) e \(f(b-\epsilon) \approx f(b)\) 
(cioè \(\epsilon > 0 \stext{ e } \pst{f(a+\epsilon)} = f(a)
\stext{ e } \pst{f(b-\epsilon)} = f(b)\)).

\affiancati{.49}{.49}{
Ad esempio la funzione rappresentata qui a fianco, è continua nell'intervallo 
