...
sull'andamento globale del fenomeno confrontando a due a due tutti i valori 
rilevati, cosa possibile proprio perché questi dati sono in numero finito.
}
{
...
\vspace{.5em}
Abbiamo detto che la curva deve essere \emph{sufficientemente regolare},
ma cosa significa?
...
\affiancati{.59}{.39}
{\scalebox{1}{\partizionen}} % TODO funzione discontinua
{\scalebox{1}{\partizionen}} % TODO funzione continua
...
\(f(x)\) è infinitamente vicino a \(f(4)\), cioè se 
\(f(4 + \epsilon) - f(4) = \delta\), dove \(\epsilon\) e \(\delta\) sono due 
infinitesimi.
...
\(f(x)\) e \(f(4)\) è infinitesima. \hfill \(\qed\) 
...
Quindi non è continua dato che \(0\) non appartiene all'insieme di 
definizione.
\end{esempio}
