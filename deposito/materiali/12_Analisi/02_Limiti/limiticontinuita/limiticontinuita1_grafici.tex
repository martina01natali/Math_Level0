...
%   \draw (\xa, \ya) -- (\xd, \ya);
  \draw (\xa, \ya) -- (\xd, \ya);
  \draw [brown!50!black, ultra thick] (\xc, \yb) -- (\xb, \yb);
  \draw [brown!50!black, ultra thick] (\xb, \ya) 
        node [below, black] {\footnotesize\(\lab\)} -- (\xb, \yb);
  \draw (\xb, \yb) -- (\xb, \yc);
}

\newcommand{\limmicxb}[9]{% 
  % interno del microscopio posto sull'asse x.
  \def \xa{#1} \def \xb{#2} \def \xc{#3} \def \xd{#4}
  \def \ya{#5} \def \yb{#6} \def \yc{#7}
  \def \laba{#8}
  \def \labb{#9}
  \draw (\xa, \ya) -- (\xd, \ya);
  \draw [blue!50!black, ultra thick] (\xb, \yb) -- (\xb, \ya)
        node [below, black] {\footnotesize\(\laba\)};
  \draw [brown!50!black, ultra thick] (\xc, \yc) -- (\xc, \ya)
        node [below, black] {\footnotesize\(\labb\)};
}

\newcommand{\limmicp}[6]{% 
  % interno del microscopio posto sull'asse x.
  \def \xa{#1} \def \xb{#2} \def \xc{#3}
  \def \ya{#4} \def \yb{#5} \def \yc{#6}
  \draw [brown!50!black, ultra thick] (\xa, \ya) -- (\xc, \ya);
  \draw [blue!50!black, ultra thick] (\xb, \ya) -- (\xb, \yb);
  \draw [green!50!black, ultra thick] (\xc, \ya) -- (\xc, \yc);
}

\newcommand{\limmicpb}[8]{% 
  % interno del microscopio posto sull'asse x.
  \def \xa{#1} \def \xb{#2} \def \xc{#3}
  \def \ya{#4} \def \yb{#5} \def \yc{#6}
  \def \laba{#7}
  \def \labb{#8}
  \draw [green!50!black, ultra thick] (\xa, \ya) -- (\xa, \yc)
        node [right, black] {\footnotesize\(\laba\)};
  \draw [black] (\xb, \yb) -- (\xa, \yb)
        node [right, black] {\footnotesize\(\labb\)};
  \draw [brown!50!black, ultra thick] (\xc, \yc) -- (\xa, \yc);
}
% 
...
\newcommand{\polregincirc}[2]{% 
  ...
  \pgfmathparse{\ang / 2} \let\sang\pgfmathresult
  \draw [thick, orange!50!black] circle(\ra);
  \draw [thick, blue!50!black] 
    (\sang:\ra) 
    \foreach \x in {1,...,\nlati} {-- (\x*\ang+\sang:\ra)} -- cycle;
  \draw [thick, green!50!black] 
    (\sang:\rb) 
    \foreach \x in {1,...,\nlati} {-- (\x*\ang+\sang:\rb)} -- cycle;
  \draw (0, 0) -- (0:\ra) (0, 0) -- (\sang:\rb);
  \draw (\ra*.2, 0) arc(0:\ang/2:\ra*.2);
  \node at (\sang/2:\ra*.2) [right] {\footnotesize \(x\)};
}

\newcommand{\agnesia}{% 
  % Riproduzione disegno Agnesi (atti VII p. 106).
  \def \ra{7}
  \def \ang{25}
  \def \rang{2}
  \disegno{
    ...
    \begin{scope}[thick]
      ...
      \draw (0, 0) coordinate [label=below left:\(A\)] (A)
      ...
      \draw[dashed]
        ...
        (D) -- (\ra, \sinang) coordinate [label=right:\(F\)] (F)
...
%     \begin{scope}[thick]
%       \draw (\ra, 0) arc(0:180:\ra);
%       \frecciax{(2, -.5)}{(1, 0)}{\delta}
%       \draw % (2, -1) node [below] {\(\delta\)}
%             (0, 0) node [below left] {\(A\)}
%             (-\ra, 0) node [below] {\(G\)}
%             (+\ra, 0) node [below] {\(B \approx C\)};
%       \draw [Green!50!black, ultra thick] (0, 0) -- (\ra, 0);
%   \microscopio{(\ra, 0)}{1}{40}{230}{2}{(\ra+2, 4)}{\(\times \infty\)}
%     \end{scope}
%   }
% }
...
\newcommand{\agnesib}{% 
  \def \ra{1}
  \disegno[20]{
    \rcom{-0.8}{+1}{0}{+1.0}{gray!50, very thin, step=1}
    \draw [brown!50!black, ultra thick] (\ra, 0) arc(0:180:\ra);
    \draw (0, 0) node [below left] {\(A\)};
    \draw [-{Latex[length=2mm, width=1mm]}, black] (+.6, -.4)
          node[below] {\footnotesize \(B \sim C \sim D\)} 
          .. controls (+.6, -.2) and (1, -.3) .. (1, 0); 
    \frecciax{(.4, -.2)}{(.2, 0)}{\delta}
    \draw [brown!50!black, ultra thick] (0, 0) -- (1.42, 0);
    \microscopio{(0, 0)}{.3}{120}{320}{.5}{(-.85, 1.1)}{~}  % M5
    \limmicx{-0.12}{-0.5}{-0.05}{-0.95}{.3}{.7}{1.08}{}
    \draw [-{Latex[length=2mm, width=1mm]}, black] (-.7, -.2) node[below]
          {\footnotesize \(\sen \delta \sim \delta \sim \tan \delta\)} 
          .. controls (-.7, -.1) and (-.6, .5) .. (-.5, .5); 
    \microscopio{(1, 0)}{.3}{40}{230}{.5}{(1.0, .9)}{~}  % M1
    \limmicx{1.14}{1.6}{1.07}{1.97}{.3}{.7}{1.08}{}
    \frecciax{(1.9, .1)}{(1.6, .3)}{B \sim C}
    \frecciax{(2.1, .6)}{(1.6, .7)}{D \sim E \sim F}
    \draw (1.5, .5) node {$\delta$};
    \microscopio{(1.6, .3)}{.5}{-100}{90}{.5}
                {(1.0, -1.1)}{~}  % M2
    \limmicxb{1.13}{1.4}{1.6}{1.9}{-1}{-.19}{-.19}{B}{C}
    \microscopio{(1.6, .7)}{.3}{120}{-90}{.5}
                {(.9, 1.8)}{~}  % M3
    \limmicp{0.97}{1.3}{1.5}{1.6}{.98}{.96}
    \frecciax{(1.1, 1.4)}{(1.3, 1.6)}{D}
    \frecciax{(1.9, 1.4)}{(1.5, 1.6)}{E \sim F}
    \microscopio{(1.5, 1.6)}{.2}{80}{-100}{.5}
                {(1.1, 2.7)}{~}  % M4
    \limmicpb{1.8}{1.14}{1.16}{1.85}{2.2}{2.5}{F}{E}
    \foreach \pl [count = \il] in {(+2.05, +.85), (+2, -.4), (+2, +1.6), 
                                   (+2.2, +2.3), (-1.1, +0.7)}{
      \node at \pl {\footnotesize{\color{gray}\(M\il\)}};}
  }
}
